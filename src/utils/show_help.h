 ...
 * for that file in $pkgdatadir (typically $prefix/share/dague).
 *
...
#ifndef DAGUE_SHOW_HELP_H
#define DAGUE_SHOW_HELP_H
 ...
 */
DAGUE_DECLSPEC int dague_show_help_init(void);
 ...
 */
DAGUE_DECLSPEC int dague_show_help_finalize(void);
 ...
 * (typically $prefix/share/dague), and looks up the message
 * based on the topic, and displays it.  If want_error_header is
 * true, a header and footer of asterisks are also displayed.
 */
typedef int (*dague_show_help_fn_t)(const char *filename, const char *topic, 
                                   bool want_error_header, ...);
DAGUE_DECLSPEC extern dague_show_help_fn_t dague_show_help;

/**
 * This function does the same thing as dague_show_help(), but accepts
 * a va_list form of varargs.
 */
typedef int (*dague_show_vhelp_fn_t)(const char *filename, const char *topic, 
                                    bool want_error_header, va_list ap);
DAGUE_DECLSPEC extern dague_show_vhelp_fn_t dague_show_vhelp;

/**
 * This function does the same thing as dague_show_help(), but returns
 * its output in a string (that must be freed by the caller).
 */
DAGUE_DECLSPEC char* dague_show_help_string(const char *filename, 
                                          const char *topic, 
                                          bool want_error_header, ...);

/**
 * This function does the same thing as dague_show_help_string(), but
 * accepts a va_list form of varargs.
 */
DAGUE_DECLSPEC char* dague_show_help_vstring(const char *filename, 
                                          const char *topic, 
                                          bool want_error_header, va_list ap);
 ...
 */
DAGUE_DECLSPEC int dague_show_help_add_dir(const char *directory);
