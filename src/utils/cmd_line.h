 ...
 *
 * \code
 * dague_cmd_line_make_opt3(cmd, 'a', NULL, 'add', 1, "Add a user");
 * \endcode
 *
 ...
 * (dague_cmd_line_t) with OBJ_NEW() and then uses it to register the
 * desired parameters via dague_cmd_line_make_opt3(). Once all the
 * parameters have been registered, the user invokes
 * dague_cmd_line_parse() with the command line handle and the argv/argc
 ...
 * passed to them, etc.:
 *
 * - dague_cmd_line_get_usage_msg() returns a string suitable for "help"
 *   kinds of messages.
 * - dague_cmd_line_is_taken() returns a true or false indicating
 *   whether a given command line option was found on the command
 *   line.
 * - dague_cmd_line_get_argc() returns the number of tokens parsed on
 *   the handle.
 * - dague_cmd_line_get_argv() returns any particular string from the
 *   original argv.
 * - dague_cmd_line_get_ninsts() returns the number of times a
 *   particular option was found on a command line.
 * - dague_cmd_line_get_param() returns the Nth parameter in the Mth
 *   instance of a given parameter.
 * - dague_cmd_line_get_tail() returns an array of tokens not parsed
 *   (i.e., if the parser ran into "--" or an unrecognized token).
 *
 * Note that a shortcut to creating a large number of options exists
 * -- one can make a table of dague_cmd_line_init_t instances and the
 * table to dague_cmd_line_create().  This creates an dague_cmd_line_t
 * handle that is pre-seeded with all the options from the table
 * without the need to repeatedly invoke dague_cmd_line_make_opt3() (or
 * equivalent).  This dague_cmd_line_t instance is just like any other;
 * it is still possible to add more options via
 * dague_cmd_line_make_opt3(), etc.
 */

#ifndef DAGUE_CMD_LINE_H
 ...
 */
struct dague_cmd_line_t {
    /** Make this an OBJ handle */
    dague_object_t super;

    /** Thread safety */
    dague_atomic_lock_t lcl_mutex;

    /** List of cmd_line_option_t's (defined internally) */
    dague_list_t lcl_options;

    /** Duplicate of argc from dague_cmd_line_parse() */
    int lcl_argc;
    /** Duplicate of argv from dague_cmd_line_parse() */
    char **lcl_argv;

    /** Parsed output; list of cmd_line_param_t's (defined internally) */
    dague_list_t lcl_params;
 ...
 * Convenience typedef
 */
typedef struct dague_cmd_line_t dague_cmd_line_t;

/**
 * Data types supported by the parser
 */
enum dague_cmd_line_type_t {
    DAGUE_CMD_LINE_TYPE_NULL,
    DAGUE_CMD_LINE_TYPE_STRING,
    DAGUE_CMD_LINE_TYPE_INT,
    DAGUE_CMD_LINE_TYPE_SIZE_T,
    DAGUE_CMD_LINE_TYPE_BOOL,

    DAGUE_CMD_LINE_TYPE_MAX
};
/**
 * \internal
 *
 * Convenience typedef
 */
typedef enum dague_cmd_line_type_t dague_cmd_line_type_t;

/**
 * Datatype used to construct a command line handle; see
 * dague_cmd_line_create().
 */
struct dague_cmd_line_init_t {
    ...
    /** If an ocl_variable_dest is given, its datatype must be
        supplied as well. */
    dague_cmd_line_type_t ocl_variable_type;

    /** Description of the command line option, to be used with
        dague_cmd_line_get_usage_msg(). */
 ...
 * Convenience typedef
 */
typedef struct dague_cmd_line_init_t dague_cmd_line_init_t;

/**
 * Top-level command line handle.
 *
 * This handle is used for accessing all command line functionality
 * (i.e., all dague_cmd_line*() functions).  Multiple handles can be
 * created and simultaneously processed; each handle is independant
 * from others.
 *
 * The dague_cmd_line_t handles are [simplisticly] thread safe;
 ...
 * destructor for dague_cmd_line_t handles will free all memory
 * associated with the handle.
 */
DAGUE_DECLSPEC OBJ_CLASS_DECLARATION(dague_cmd_line_t);

/**
 * Make a command line handle from a table of initializers.
 *
 * @param cmd DAGUE command line handle.
 * @param table Table of dague_cmd_line_init_t instances for all
 * the options to be included in the resulting command line
 * handler.
 *
 * @retval DAGUE_SUCCESS Upon success.
 *
 * This function takes a table of dague_cmd_line_init_t instances
 * to pre-seed an DAGUE command line handle.  The last instance in
 ...
 *
 * \code
 * dague_cmd_line_init_t cmd_line_init[] = {
 *    { NULL, NULL, NULL, 'h', NULL, "help", 0,
 *      &orterun_globals.help, DAGUE_CMD_LINE_TYPE_BOOL,
 *      "This help message" },
 *
 *    { NULL, NULL, NULL, '\0', NULL, "wd", 1,
 *      &orterun_globals.wd, DAGUE_CMD_LINE_TYPE_STRING,
 *      "Set the working directory of the started processes" },
 *
 *    { NULL, NULL, NULL, '\0', NULL, NULL, 0,
 *      NULL, DAGUE_CMD_LINE_TYPE_NULL, NULL }
 * };
 * \endcode
 */
DAGUE_DECLSPEC int dague_cmd_line_create(dague_cmd_line_t *cmd,
                                         dague_cmd_line_init_t *table);
 ...
 *
 */
DAGUE_DECLSPEC int dague_cmd_line_make_opt_mca(dague_cmd_line_t *cmd,
 ...
 *
 * @retval DAGUE_ERR_OUT_OF_RESOURCE If out of memory.
 * @retval DAGUE_ERR_BAD_PARAM If bad parameters passed.
 * @retval DAGUE_SUCCESS Upon success.
 *
 * Adds a command line option to the list of options that an DAGUE
 ...
 * used to generate the output from dague_cmd_line_get_usage_msg().
 *
 */
DAGUE_DECLSPEC int dague_cmd_line_make_opt3(dague_cmd_line_t *cmd,
                                            char short_name,
                                            const char *sd_name,
...
/**
 * Parse a command line according to a pre-built DAGUE command line
 * handle.
 *
 * @param cmd DAGUE command line handle.
 ...
 *
 * @retval DAGUE_SUCCESS Upon success.
 * @retval DAGUE_ERR_SILENT If an error message was printed.  This
 * value will only be returned if the command line was not
 * successfully parsed.
 *
 * Parse a series of command line tokens according to the option
 * descriptions from a DAGUE command line handle.  The DAGUE command line
 ...
 * of ignore_unknown (to stderr, and DAGUE_ERR_SILENT is
 * returned) if:
 *
 ...
 * tokens is available from the dague_cmd_line_get_tail() function.
 *
 ...
 */
DAGUE_DECLSPEC int dague_cmd_line_parse(dague_cmd_line_t *cmd,
                                        bool ignore_unknown,
                                        int argc, char **argv);

/**
 * Return a consolidated "usage" message for a DAGUE command line handle.
 *
 * @param cmd DAGUE command line handle.
 *
 ...
 * the DAGUE command line handle.  Options that passed a NULL
 * description to dague_cmd_line_make_opt3() will not be included in the
 * display (to allow for undocumented options).
 *
 * This function is typically only invoked internally by the
 * dague_show_help() function.
 *
 ...
 */
DAGUE_DECLSPEC char *dague_cmd_line_get_usage_msg(dague_cmd_line_t *cmd);
 ...
 * dague_cmd_line_parse().
 *
 * @retval false If the command line option was not found during
 * dague_cmd_line_parse(), or dague_cmd_line_parse() was not invoked on
 * this handle.
 *
 * This function should only be called after dague_cmd_line_parse().
 *
 ...
 */
DAGUE_DECLSPEC bool dague_cmd_line_is_taken(dague_cmd_line_t *cmd, const char *opt);

/**
 * Return the number of arguments parsed on a DAGUE command line handle.
 *
 * @param cmd A pointer to the DAGUE command line handle.
 *
 * @retval DAGUE_ERROR If cmd is NULL.
 * @retval argc Number of arguments previously added to the handle.
 *
 * Arguments are added to the handle via the dague_cmd_line_parse()
 * function.
 */
DAGUE_DECLSPEC int dague_cmd_line_get_argc(dague_cmd_line_t *cmd);

/**
 * Return a string argument parsed on a DAGUE command line handle.
 *
 * @param cmd A pointer to the DAGUE command line handle.
 ...
 * found during dague_cmd_line_parse().
 *
 * @retval DAGUE_ERR If the command line option was not found during
 * dague_cmd_line_parse(), or dague_cmd_line_parse() was not invoked on
 * this handle.
 *
 * This function should only be called after dague_cmd_line_parse().
 ...
 * (either by its short or long name) -- to include 0 -- or DAGUE_ERR if
 * either the option was not specified as part of the DAGUE command line
 * handle, or dague_cmd_line_parse() was not invoked on this handle.
 */
DAGUE_DECLSPEC int dague_cmd_line_get_ninsts(dague_cmd_line_t *cmd, const char *opt);
 ...
 *
 * The call to dague_cmd_line_get_param(cmd, "foo", 1, 1) would return
 * "bar4".  dague_cmd_line_get_param(cmd, "bar", 0, 0) would return
 * NULL, as would dague_cmd_line_get_param(cmd, "foo", 2, 2);
 *
 * The returned string should \em not be modified or freed by the
 * caller.
 */
DAGUE_DECLSPEC char *dague_cmd_line_get_param(dague_cmd_line_t *cmd,
                                              const char *opt,
                                              int instance_num,
                                              int param_num);

/**
 * Return the entire "tail" of unprocessed argv from a DAGUE
 * command line handle.
 *
 * @param cmd A pointer to the DAGUE command line handle.
 ...
 *
 * @retval DAGUE_ERROR If cmd is NULL or otherwise invalid.
 * @retval DAGUE_SUCCESS Upon success.
 *
 ...
 * to dague_argv_free()) by the caller.
 */
DAGUE_DECLSPEC int dague_cmd_line_get_tail(dague_cmd_line_t *cmd, int *tailc,
                                           char ***tailv);

END_C_DECLS


#endif /* DAGUE_CMD_LINE_H */
