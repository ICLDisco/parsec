#include "pins_task_select_papi_core.h"
...
static int task_select_events[NUM_TASK_SELECT_EVENTS] = {PAPI_L2_TCM, PAPI_L2_DCM};

#define THREAD_NUM(exec_unit) (exec_unit->virtual_process->vp_id *       \
                              exec_unit->virtual_process->dague_context->nb_vp + \
                              exec_unit->th_id )

void pins_init_task_select_papi_core(dague_context_t * master) {
	PINS_REGISTER(TASK_SELECT_BEGIN, start_task_select_papi_count);
	PINS_REGISTER(TASK_SELECT_END, stop_task_select_papi_count);
}
...
void pins_thread_init_task_select_papi_core(dague_execution_unit_t * exec_unit) {
	...
	exec_unit->papi_eventsets[TASK_SELECT_SET] = PAPI_NULL;
	if (PAPI_create_eventset(&exec_unit->papi_eventsets[TASK_SELECT_SET]) != PAPI_OK)
		printf("task_select_papi_core.c, pins_thread_init_task_select_papi_core: failed to create StealEventSet\n");
	if ((rv = PAPI_add_events(exec_unit->papi_eventsets[TASK_SELECT_SET], task_select_events, NUM_TASK_SELECT_EVENTS)) != PAPI_OK)
		printf("task_select_papi_core.c, pins_thread_init_task_select_papi_core: failed to add "
...
void start_task_select_papi_count(dague_execution_unit_t * exec_unit, dague_execution_context_t * exec_context, void * data) {
	...
	if ((rv = PAPI_start(exec_unit->papi_eventsets[TASK_SELECT_SET])) != PAPI_OK)
		printf("%p task_select_papi_core.c, start_task_select_papi_count: can't start steal event counters! %d %s\n", exec_unit, rv, PAPI_strerror(rv));
	else
...
void stop_task_select_papi_count(dague_execution_unit_t * exec_unit, dague_execution_context_t * exec_context, void * data) {
	...
	if ((rv = PAPI_stop(exec_unit->papi_eventsets[TASK_SELECT_SET], values)) != PAPI_OK) {
		printf("task_select_papi_core.c, stop_task_select_papi_count: can't stop steal event counters! %d %s\n", rv, PAPI_strerror(rv));
