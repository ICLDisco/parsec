...
#ifndef DAGUE_VALUE_ARRAY_H
#define DAGUE_VALUE_ARRAY_H
...
typedef struct dague_value_array_t dague_value_array_t;

DAGUE_DECLSPEC OBJ_CLASS_DECLARATION(dague_value_array_t);
...
static inline int dague_value_array_init(dague_value_array_t *array, size_t item_sizeof)
{
    ...
    return (NULL != array->array_items) ? DAGUE_SUCCESS : DAGUE_ERR_OUT_OF_RESOURCE;
}
...
static inline int dague_value_array_reserve(dague_value_array_t* array, size_t size)
{
...
static inline size_t dague_value_array_get_size(dague_value_array_t* array)
{
 ...
 *  case, if the routine is successful, dague_value_array_get_size() will 
 *  return the new size.
 */

DAGUE_DECLSPEC int dague_value_array_set_size(dague_value_array_t* array, size_t size);
...
#define DAGUE_VALUE_ARRAY_GET_ITEM(array, item_type, item_index) \
...
static inline void* dague_value_array_get_item(dague_value_array_t *array, size_t item_index)
{
    if(item_index >= array->array_size && dague_value_array_set_size(array, item_index+1) != DAGUE_SUCCESS)
        return NULL;
...
#define DAGUE_VALUE_ARRAY_SET_ITEM(array, item_type, item_index, item_value) \
...
static inline int dague_value_array_set_item(dague_value_array_t *array, size_t item_index, const void* item)
{
    int rc;
    if(item_index >= array->array_size && 
       (rc = dague_value_array_set_size(array, item_index+1)) != DAGUE_SUCCESS)
        return rc;
...
static inline int dague_value_array_append_item(dague_value_array_t *array, const void *item)
{
    return dague_value_array_set_item(array, array->array_size, item);
}
 ...
 * than looping over DAGUE_VALUE_ARRAY_GET_ITEM() and
 * DAGUE_VALUE_ARRAY_SET_ITEM() because it will [potentially] reduce the
 * number of pointer dereferences.
 */

#define DAGUE_VALUE_ARRAY_GET_BASE(array, item_type) \
