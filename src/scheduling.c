...
{
    assert( 0 == dague_object->nb_functions );
    dague_compound_state_t* compound_state = (dague_compound_state_t*)dague_object->functions_array;
    ...
    dague_object->nb_local_tasks = compound_state->nb_objects;
}
                               ...
                               dague_object_t* next ) 
{
    ...
    if( start->nb_functions == 0 ) {
        ...
        next->complete_cb = dague_composed_cb;
        next->complete_cb_data = compound;
        ...
        start->complete_cb = next->complete_cb = dague_composed_cb;
        start->complete_cb_data = next->complete_cb_data = compound; 
    }
    return compound;
}
