...
/** obtain the first item of @list, or NULL if @list is empty, 
...
/** obtain the last item of @list, or NULL if @list is empty,
 *    the returned item remains in the list (not thread safe) */
static inline dague_list_item_t*
dague_list_iterate_last( dague_list_t* list );
/** obtain the successor of @item, or NULL if @item is the tail of @list
 *    the returned item remains in the list (not thread safe) */
static inline dague_list_item_t*
dague_list_iterate_next( dague_list_t* list, 
                         dague_list_item_t* item );
/** obtain the predecessor of @item, or NULL if @item is the head of @list
 *    the returned item remains in the list (not thread safe) */
static inline dague_list_item_t*
dague_list_iterate_prev( dague_list_t* list, 
                         dague_list_item_t* item );
/** add the @new item before the @position item in @list (not thread safe)
 *    @position item is obtained by one of the iterate functions
...
#define _NULL_GHOST(LIST, ITEM) (((&(LIST)->ghost_element) == item)? NULL: item)
...
dague_list_pop_front( dague_list_t* list )
{
    dague_list_item_t* item;
    dague_atomic_lock(&list->atomic_lock);
    item = dague_list_nolock_pop_front(list);
    dague_atomic_unlock(&list->atomic_lock);
    return item;
}
...
dague_list_pop_back( dague_list_t* list )
{
    dague_list_item_t* item;
    dague_atomic_lock(&list->atomic_lock);
    item = dague_list_nolock_pop_back(list);
    dague_atomic_unlock(&list->atomic_lock);
    return item;
}
...
{
    dague_atomic_lock(&list->atomic_lock);
    item = dague_list_nolock_remove_item(list, item);
    dague_atomic_unlock(&list->atomic_lock);
    return item;
}
