 ...
 *
 */
extern int dplasma_hwloc_master_id( int level, int processor_id );
 
/**
 * Find the number of core for master_id at n level 
 *
 */
extern unsigned int dplasma_hwloc_nb_cores( int level, int master_id );
 
/**
 * Find the number of level from the computer architectur
 *
 */
extern int dplasma_hwloc_nb_levels( void );
    
/**
 * Find the cache size for master at n level
 *
 */
extern size_t dplasma_hwloc_cache_size( int level, int master_id );

/**
 * Find the distance between id1 and id2
 *
 */
extern int dplasma_hwloc_distance( int id1, int id2 );

/**
 * load the HWLOC topology.
 */
extern int dplasma_hwloc_init(void);

/**
 * unload the HWLOC topology.
 */
extern int dplasma_hwloc_fini(void);
