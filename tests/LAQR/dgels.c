...
#if defined(HAVE_GETOPT_LONG)
        int option_index = 0;
        c = getopt_long (argc, argv, "dpxc:N:M:a:r:b:g:e:s:w::B:I:h",
                         long_options, &option_index);
#else
        c = getopt (argc, argv, "dpxc:N:M:a:r:b:g:e:s:w::B:I:h");
#endif  /* defined(HAVE_GETOPT_LONG) */
...
#define B2      (*pB2)
    ...
    if(do_nasty_validations)
    {
        ...
        lapack_dlarnv(IONE, ISEED, LDB*NRHS, B1);
        for (i = 0; i < M; i++)
            for (j = 0; j < NRHS; j++)
...
{    
    ...
    {
        int i, K = min(M, N);
        double *Q  = (double *)malloc(LDA*K*sizeof(double));
...
int check_orthogonality(int M, int N, int LDQ, double *Q, double eps)
{
    ...
    normQ = lapack_dlansy(lapack_inf_norm, (enum lapack_uplo_type)(PlasmaUpper), minMN, Id, minMN, work);
...
int check_factorization(int M, int N, double *A1, double *A2, int LDA, double *Q, double eps )
{
    ...
    if (M >= N) {
        ...
        lapack_dlacpy(lapack_upper, M, N, A2, LDA, R, N);
        ...
        lapack_dlacpy(lapack_lower, M, N, A2, LDA, L, M);
    ...
    Rnorm = lapack_dlange( lapack_inf_norm, M, N, Residual,   M, work);
    Anorm = lapack_dlange( lapack_inf_norm, M, N,       A2, LDA, work);
...
int check_solution(int M, int N, int NRHS, double *A1, int LDA, double *B1, double *B2, int LDB, double eps)
{
    ...
    Anorm = lapack_dlange(lapack_inf_norm, M, N,    A1, LDA, work);
    Xnorm = lapack_dlange(lapack_inf_norm, M, NRHS, B2, LDB, work);
    ...
    if (M >= N) {
       ...
       Rnorm = lapack_dlange(lapack_inf_norm, M, NRHS, Residual, M, work);
       free(Residual);
    }
    else {
