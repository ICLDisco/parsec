...
{
    parsec_pcb_taskpool_t *tp = NULL;

    if( nt <= 0 || level <= 0 ) {
        fprintf(stderr, "To work, PCB must have at least one task to run per level\n");
        return (parsec_taskpool_t*)tp;
    }

    tp = parsec_pcb_new(nt, level, A);
...
#if defined(PARSEC_HAVE_MPI)
    {
        ...
        parsec_arena_datatype_construct( &tp->arenas_datatypes[PARSEC_pcb_DEFAULT_ADT_IDX],
