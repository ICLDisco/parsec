...
extern void cuda_pong_kernel(int *dev_data, int idx, void *stream);
...
#if defined(PARSEC_HAVE_DEV_HIP_SUPPORT)
extern void hip_pong_kernel(int *dev_data, int idx, void *stream);
...
int main(int argc, char **argv)
{
    ...
    parsec_task_class_t *ping_tc = parsec_dtd_create_task_class(dtd_tp, "ping",
                                                                ...
                                                                PARSEC_DTD_ARG_END);
    assert(NULL != ping_tc);
    rc = parsec_dtd_task_class_add_chore(dtd_tp, ping_tc, PARSEC_DEV_CPU, cpu_ping);
    ...
    parsec_task_class_t *pong_tc = parsec_dtd_create_task_class(dtd_tp, "pong",
                                                                ...
                                                                PARSEC_DTD_ARG_END);
    assert(NULL != pong_tc);
    rc = parsec_dtd_task_class_add_chore(dtd_tp, pong_tc, PARSEC_DEV_CPU, cpu_pong);
    PARSEC_CHECK_ERROR(rc, "parsec_dtd_task_class_add_chore");
#if defined(PARSEC_HAVE_DEV_CUDA_SUPPORT)
    rc = parsec_dtd_task_class_add_chore(dtd_tp, pong_tc, PARSEC_DEV_CUDA, cuda_pong);
    PARSEC_CHECK_ERROR(rc, "parsec_dtd_task_class_add_chore");
#endif
#if defined(PARSEC_HAVE_DEV_HIP_SUPPORT)
    rc = parsec_dtd_task_class_add_chore(dtd_tp, pong_tc, PARSEC_DEV_HIP, hip_pong);
    PARSEC_CHECK_ERROR(rc, "parsec_dtd_task_class_add_chore");
#endif
    ...
    parsec_dtd_task_class_release(dtd_tp, pong_tc);
