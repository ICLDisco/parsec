...
static dague_object_t *dague_sgeqrt = NULL;
...
extern int sgeqrt_private_memory_initialization(int MB, int NB );
...
#if defined(DAGUE_CUDA_SUPPORT)
    static char shortopts[] = "c:N:M:a:r:b:g:e:s:B:I:hG:";
#else
    static char shortopts[] = "c:N:M:a:r:b:g:e:s:B:I:h";
#endif
...
static dague_context_t *setup_dague(int* pargc, char** pargv[])
{
    ...
    dague_sgeqrt = (dague_object_t*)dague_sgeqrt_new((dague_ddesc_t*)&ddescT, (dague_ddesc_t*)&ddescA, 
                                                     MB, NB, M, N, 
                                                     MT, NT, MINMTNT);
    dague_enqueue( dague, (dague_object_t*)dague_sgeqrt);

    nbtasks = dague_sgeqrt->nb_local_tasks;
    printf("sgeqrt %ux%u has %u tasks to run. Total nb tasks to run: %u\n", 
           ddescA.super.nb, ddescA.super.nt, dague_sgeqrt->nb_local_tasks, dague->taskstodo);
    printf("GRIDrows = %u, GRIDcols = %u, rrank = %u, crank = %u\n", 
           ddescA.GRIDrows, ddescA.GRIDcols, ddescA.rowRANK, ddescA.colRANK );
    
    sgeqrt_private_memory_initialization(MB, NB);
...
static void cleanup_dague(dague_context_t* dague)
{
    ...
    asprintf( &filename, "%s.%d.profile", "sgeqrt", rank );
