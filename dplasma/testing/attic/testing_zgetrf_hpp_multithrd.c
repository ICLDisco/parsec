...
int main(int argc, char ** argv)
{
    ...
    dplasma_zplrnt( parsec, (tiled_matrix_desc_t *)&ddescA, 3872);
    dplasma_zlaset( parsec, PlasmaUpperLower, 0., 0., (tiled_matrix_desc_t *)&ddescLT);
    ...
    if ( check ) {
        ...
        dplasma_zplrnt( parsec, (tiled_matrix_desc_t *)&ddescB, 2354 );
        dplasma_zlacpy( parsec, PlasmaUpperLower,
                        (tiled_matrix_desc_t *)&ddescB,
                        (tiled_matrix_desc_t *)&ddescX );
    }
    ...
    /* Create PaRSEC */
    if(loud > 2) printf("+++ Computing zgetrf_hpp_multithrd ... ");
    PASTE_CODE_ENQUEUE_KERNEL(parsec, zgetrf_hpp_multithrd,
                              (qrpiv,
                               (tiled_matrix_desc_t*)&ddescA,
                               ...
                               &info));
    /* lets rock! */
    PASTE_CODE_PROGRESS_KERNEL(parsec, zgetrf_hpp_multithrd);
    dplasma_zgetrf_hpp_Destruct( PARSEC_zgetrf_hpp_multithrd );
    if(loud > 2) printf("Done.\n");
    ...
    if ( info != 0 ) {
         ...
         */
        dplasma_ztrsmpl_hpp( parsec, qrpiv,
                             (tiled_matrix_desc_t *)&ddescA,
                             (tiled_matrix_desc_t *)&ddescX,
                             (tiled_matrix_desc_t *)&ddescIPIV,
                             (tiled_matrix_desc_t *)&ddescLT,
    ...
    if ( check ) {
        ...
        if ( check_inv ) {
            ...
            parsec_ddesc_destroy((parsec_ddesc_t*)&ddescInvA);
            parsec_data_free(ddescI.mat);
            parsec_ddesc_destroy((parsec_ddesc_t*)&ddescI);
        }
    }
    ...
    parsec_data_free(ddescLT.mat);
    parsec_ddesc_destroy((parsec_ddesc_t*)&ddescLT);
