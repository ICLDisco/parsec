                       ...
                       CUstream stream )
{
     ...
     */
    dague_gpu_data_get_elt(args->ddescA->moesi_map, GEMM_KEY(args->ddescA, args->Am, args->An ),
                           &(this_task->data[0].mem2dev_data));
    if( NULL == (this_task->data[0].mem2dev_data)->device_elem[gpu_device->index])
        move_data_count++;

    dague_gpu_data_get_elt(args->ddescB->moesi_map, GEMM_KEY(args->ddescB, args->Bm, args->Bn ),
                           &(this_task->data[1].mem2dev_data));
    if( NULL == (this_task->data[1].mem2dev_data)->device_elem[gpu_device->index])
        move_data_count++;

    dague_gpu_data_get_elt(args->ddescC->moesi_map, GEMM_KEY(args->ddescC, args->Cm, args->Cn ),
               ...
               dague_complex64_t beta,  int Cm, int Cn, const tiled_matrix_desc_t *descC, int ldc )
{
    ...
    which_gpu = dague_gpu_data_elt_write_owner( descC->super.moesi_map, GEMM_KEY( descC, Cm, Cn) );
