...
dplasma_zpotrf_New(PLASMA_enum uplo, tiled_matrix_desc_t *A, int *info)
{
    ...
    *info = 0;
    if ( uplo == PlasmaUpper ) {
        dague_zpotrf = (dague_handle_t*)dague_zpotrf_Url_new( uplo, *A, (dague_ddesc_t*)A, info);
    } else {
        dague_zpotrf = (dague_handle_t*)dague_zpotrf_Lrl_new( uplo, *A, (dague_ddesc_t*)A, info);
    }
    ((struct dague_zpotrf_Lrl_handle*)dague_zpotrf)->PRI_CHANGE = dplasma_aux_get_priority_limit( "POTRF", A );

    dplasma_add2arena_tile(((dague_zpotrf_Url_handle_t*)dague_zpotrf)->arenas[DAGUE_zpotrf_Url_DEFAULT_ARENA],
...
{
    dague_zpotrf_Url_handle_t *opotrf = (dague_zpotrf_Url_handle_t *)o;
