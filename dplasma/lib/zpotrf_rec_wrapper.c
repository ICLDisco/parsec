...
#include "zpotrf_U_rec.h"
#include "zpotrf_L_rec.h"
 ...
 ******************************************************************************/
dague_handle_t*
dplasma_zpotrf_rec_New( PLASMA_enum uplo,
                        ...
                        int *info )
{
    ...
    *info = 0;
    if ( uplo == PlasmaUpper ) {
        o = (dague_handle_t*)dague_zpotrf_U_rec_new( uplo, (dague_ddesc_t*)A, info);
    } else {
        o = (dague_handle_t*)dague_zpotrf_L_rec_new( uplo, (dague_ddesc_t*)A, info);
    }

    dague_zpotrf = (dague_zpotrf_L_rec_handle_t*)o;
    dague_zpotrf->PRI_CHANGE = dplasma_aux_get_priority_limit( "POTRF", A );

    dplasma_add2arena_tile( dague_zpotrf->arenas[DAGUE_zpotrf_L_rec_DEFAULT_ARENA],
