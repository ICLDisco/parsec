 ...
 *
 *  dplasma_zunglq_New - Generates the dague handle that computes the generation
                    ...
                    tiled_matrix_desc_t *Q )
{
    ...
    /* Default type */
    dplasma_add2arena_tile( handle->arenas[DAGUE_zunglq_DEFAULT_ARENA],
                            A->mb*A->nb*sizeof(dague_complex64_t),
                            DAGUE_ARENA_ALIGNMENT_SSE,
                            dague_datatype_double_complex_t, A->mb );

    /* Upper triangular part of tile without diagonal */
    dplasma_add2arena_upper( handle->arenas[DAGUE_zunglq_UPPER_TILE_ARENA],
                             A->mb*A->nb*sizeof(dague_complex64_t),
                             DAGUE_ARENA_ALIGNMENT_SSE,
                             dague_datatype_double_complex_t, A->mb, 0 );

    /* Little T */
    dplasma_add2arena_rectangle( handle->arenas[DAGUE_zunglq_LITTLE_T_ARENA],
 ...
 ******************************************************************************/
void
dplasma_zunglq_Destruct( dague_handle_t *handle )
{
    dague_zunglq_handle_t *dague_zunglq = (dague_zunglq_handle_t *)handle;

    dague_matrix_del2arena( dague_zunglq->arenas[DAGUE_zunglq_DEFAULT_ARENA   ] );
    dague_matrix_del2arena( dague_zunglq->arenas[DAGUE_zunglq_UPPER_TILE_ARENA] );
    dague_matrix_del2arena( dague_zunglq->arenas[DAGUE_zunglq_LITTLE_T_ARENA  ] );

    dague_private_memory_fini( dague_zunglq->_g_p_work );
    free( dague_zunglq->_g_p_work );
