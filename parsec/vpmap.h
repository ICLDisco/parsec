...
/**
 * @return the total number of threads across all local virtual processes
 */
int parsec_vpmap_get_nb_total_threads(void);
 ...
 * @return the number of binding resources on which this VP thread can be bound
 ...
 * @param thread: identifier of the thread
 *        0 <= thread < vpmap_get_nb_threads_in_vp(vp)
 * @param ht: output hyper-thread index associated with this VP thread.
 * @return the candidate binding-resource mask for this VP thread. The mask is
 *         expressed in PaRSEC binding-resource indexes, and parsec.c selects
 *         the concrete resource from this mask unless bind_map explicitly
