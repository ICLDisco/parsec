...
parsec_dtd_ordering_correctly( parsec_execution_stream_t *es,
                               const parsec_task_t *_this_task,
                               uint32_t action_mask,
                               parsec_ontask_function_t *ontask,
                               void *ontask_arg )
{
    parsec_dtd_task_t *this_task = (parsec_dtd_task_t *)_this_task;
    parsec_dtd_task_t *current_desc = NULL;
    parsec_dtd_flow_info_t* flow;
    int op_type_on_current_flow, desc_op_type, desc_flow_index, current_dep;
    ...
    parsec_dtd_descendant_info_t* successor;

    rank_src = this_task->rank;
    for( current_dep = 0; current_dep < this_task->super.task_class->nb_flows; current_dep++ ) {
        if( !(action_mask & (1 << current_dep)) ) {
            continue;
        }

        flow = FLOW_OF(this_task, current_dep);
        if( NULL == (tile = flow->tile) ) {
            continue;
        }
        successor = DESC_OF(this_task, current_dep);
        current_desc = successor->task;
        op_type_on_current_flow = (flow->op_type & PARSEC_GET_OP_TYPE);

        if( FLOW_OF(this_task, current_dep)->op_type & PARSEC_DONT_TRACK ) {
        ...
        if(action_mask & PARSEC_ACTION_RELEASE_LOCAL_DEPS) {
            if( PARSEC_INPUT == op_type_on_current_flow ) {
                if(parsec_dtd_task_is_local(this_task)){
                    (void)parsec_atomic_fetch_dec_int32( &this_task->super.data[current_dep].data_out->readers );
                }
            }
        }
         ...
         * to ourself.
         */
        if(this_task == current_desc) {
            if(parsec_dtd_task_is_local(current_desc)) {
                current_desc->super.data[successor->flow_index].data_in = this_task->super.data[current_dep].data_out;
                current_desc->super.data[successor->flow_index].data_out = this_task->super.data[current_dep].data_out;
                parsec_dtd_retain_data_copy(this_task->super.data[current_dep].data_out);
            }
            continue;
        }
        ...
        if( NULL == current_desc ) {
            if( PARSEC_INOUT == op_type_on_current_flow ||
                ...
                if(action_mask & PARSEC_ACTION_RELEASE_LOCAL_DEPS) {
                    if(release_ownership_of_data(this_task, current_dep)) { /* trying to release ownership */
                ...
                if(action_mask & PARSEC_ACTION_RELEASE_LOCAL_DEPS) {
                    if( flow->flags & RELEASE_OWNERSHIP_SPECIAL ) {
                        made_sure_nextinline_is_null(this_task, current_dep);
                    }
                }
                continue;
            }
        ...
        /* setting data */
        data.data   = this_task->super.data[current_dep].data_out;
        adt = parsec_dtd_get_arena_datatype(this_task->super.taskpool->context,
                                            flow->arena_index);
        if( NULL != adt ) {
        ...
        desc_op_type = successor->op_type & PARSEC_GET_OP_TYPE;
        desc_flow_index = successor->flow_index;
        ...
        do {
            ...
            if(action_mask & PARSEC_ACTION_RELEASE_LOCAL_DEPS) {
                if(parsec_dtd_task_is_local(current_desc)) {
                    current_desc->super.data[desc_flow_index].data_in = this_task->super.data[current_dep].data_out;
            ...
            if( !(PARSEC_OUTPUT == desc_op_type || PARSEC_INOUT == desc_op_type) ) {
              ...
              look_for_next:
                ...
                if(action_mask & PARSEC_ACTION_RELEASE_LOCAL_DEPS) {
                    if(parsec_dtd_task_is_local(current_desc)){
                        (void)parsec_atomic_fetch_inc_int32( &this_task->super.data[current_dep].data_out->readers );
                    }
                }
                ...
                if(action_mask & PARSEC_ACTION_RELEASE_LOCAL_DEPS) {
                    if( !(FLOW_OF(this_task, current_dep)->flags & SUCCESSOR_ITERATED) ){
                        FLOW_OF(this_task, current_dep)->flags |= SUCCESSOR_ITERATED;
                    }
            ...
            PARSEC_DEBUG_VERBOSE(parsec_dtd_dump_traversal_info, parsec_dtd_debug_output,
                                 ...
                                 this_task->ht_item.key, this_task->rank, current_desc->super.task_class->name,
            ...
            deps.belongs_to      = this_task->super.task_class->out[current_dep];
            deps.direct_data     = NULL;
            deps.dep_datatype_index = current_dep;

            rank_dst = current_desc->rank;

            ontask( es, (parsec_task_t *)current_desc, (parsec_task_t *)this_task,
                    &deps, &data, rank_src, rank_dst, vpid_dst, NULL, 0, ontask_arg );
            vpid_dst = (vpid_dst+1) % this_task->super.taskpool->context->nb_vp;

            /* releasing remote tasks that is a descendant of a local task */
            if(action_mask & PARSEC_ACTION_RELEASE_LOCAL_DEPS) {
                if( parsec_dtd_task_is_remote(current_desc) && parsec_dtd_task_is_local(this_task) ) {
                    parsec_dtd_remote_task_release( current_desc );
                }
                if(release_parent) {
                    if( parsec_dtd_task_is_local(this_task) ) {
