 ...
 * for that file in $pkgdatadir (typically $prefix/share/parsec).
 *
 ...
 */
PARSEC_DECLSPEC int parsec_show_help_init(void);
 ...
 */
PARSEC_DECLSPEC int parsec_show_help_finalize(void);
 ...
 * (typically $prefix/share/parsec), and looks up the message
...
/**
 * This function does the same thing as parsec_show_help(), but accepts
...
/**
 * This function does the same thing as parsec_show_help(), but returns
 ...
 * needs to tell show_help how to find its own show_help files - without
 * interfering with the linked ORTE libs when they need to do show_help.
 */
PARSEC_DECLSPEC int parsec_show_help_add_dir(const char *directory);
