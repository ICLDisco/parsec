...
/** @file
 * PARSEC output stream facility.
 *
 * The PARSEC output stream facility is used to send output from the PARSEC
 ...
 * parsec_output_open() function call, and then sent output via
 * parsec_output_verbose(), PARSEC_OUTPUT(), and parsec_output().  Streams are
 * closed with parsec_output_close().
 *
 ...
 * - file
 *
 * Which outputs to use are specified during parsec_output_open().
 *
 ...
 * not be able to write to a file for output.  See parsec_output() for
 * details on what happens in these cases.
 *
 * parsec_output_open() returns an integer handle that is used in
 * successive calls to PARSEC_OUTPUT() and parsec_output() to send output to
 * the stream.
 *
 * The default "verbose" stream is opened after invoking
 * parsec_output_init() (and closed after invoking
 * parsec_output_finalize()).  This stream outputs to stderr only, and
 ...
 * output (both from parsec_output and stdout/stderr of processes) to
 * syslog:
 *
 * PARSEC_OUTPUT_REDIRECT - set to "syslog" to redirect to syslog. Other
 *                        options may someday be supported
 * PARSEC_OUTPUT_SYSLOG_PRI - set to "info", "error", or "warn" to have
 *                        output sent to syslog at that priority
 * PARSEC_OUTPUT_SYSLOG_IDENT - a string identifier for the log
 *
 ...
 */
PARSEC_DECLSPEC extern bool parsec_output_redirected_to_syslog;
PARSEC_DECLSPEC extern int parsec_output_redirected_syslog_pri;

/**
 * \class parsec_output_stream_t
 *
 * Structure used to request the opening of a PARSEC output stream.  A
 * pointer to this structure is passed to parsec_output_open() to tell
 * the parsec_output subsystem where to send output for a given stream.
 ...
 * destinations through the parsec_output facility.
 *
 ...
 * from parsec_output_open().
 */
struct parsec_output_stream_t {
    /** Class parent */
     ...
     * parsec_output_verbose() function.  Verbose levels are ignored in
     * PARSEC_OUTPUT() and parsec_output().
     *
    ...
    /**
     * When parsec_output_stream_t::lds_want_syslog is true, this field is
     ...
     * to an internal structure in the call to parsec_output_open()!
     */
    char *lds_prefix;
     ...
     * the stream unless PARSEC was configured with PARSEC_DEBUG_NOISIER.
     */
    ...
    bool lds_want_file;
    /**
     * When parsec_output_stream_t::lds_want_file is true, this field
     ...
     * prefix set by parsec_output_set_output_file_info() (e.g.,
     ...
     * exist when parsec_output_open() is invoked.  See parsec_output()
 ...
 * Convenience typedef
 */
typedef struct parsec_output_stream_t parsec_output_stream_t;
 ...
 * and can be written to via calls to parsec_output_verbose() and
 * parsec_output_error().
 *
 * By definition, the default verbose stream has a handle ID of 0,
 * and has a verbose level of 0.
 */
PARSEC_DECLSPEC bool parsec_output_init(void);
 ...
 * stream.
 */
PARSEC_DECLSPEC void parsec_output_finalize(void);

/**
 * Opens an output stream.
 *
 * @param lds A pointer to parsec_output_stream_t describing what the
 ...
 * using it in successive calls to PARSEC_OUTPUT(), parsec_output(),
 * parsec_output_switch(), and parsec_output_close().
 *
 ...
 * unspecified manner.
 *
 * Be sure to see parsec_output() for a description of what happens
 * when open_open() / parsec_output() is directed to send output to a
 * file but the process session directory does not yet exist.
 */
PARSEC_DECLSPEC int parsec_output_open(parsec_output_stream_t *lds);
 ...
 */
PARSEC_DECLSPEC int parsec_output_reopen(int output_id, parsec_output_stream_t *lds);
 ...
 * the parsec_output subsystem to intercept and discard any output sent
 * to the stream via PARSEC_OUTPUT() or parsec_output() until the output
 * is re-enabled.
 */
PARSEC_DECLSPEC bool parsec_output_switch(int output_id, bool enable);
 ...
 */
PARSEC_DECLSPEC void parsec_output_reopen_all(void);
 ...
 * parsec_output_verbose() and parsec_output_error()).
 *
 ...
 * session directory does not exist when parsec_output() is invoked,
 * the output will be discarded!  Once the session directory is
 * created, parsec_output() will automatically create the file and
 * writing to it.
 */
PARSEC_DECLSPEC void parsec_output(int output_id, const char *format, ...);
 ...
 * function will effectively invoke parsec_output to send the output to
 * the stream.
 *
 * @see parsec_output_set_verbosity()
 */
PARSEC_DECLSPEC void parsec_output_verbose(int verbose_level, int output_id,
                                         const char *format, ...);

/**
 * Same as parsec_output_verbose(), but takes a va_list form of varargs.
 */
PARSEC_DECLSPEC void parsec_output_vverbose(int verbose_level, int output_id,
 ...
 */
PARSEC_DECLSPEC char *parsec_output_vstring(int verbose_level, int output_id,
                                          const char *format, va_list ap);
 ...
 * will be used for all future invocations of parsec_output_verbose().
 */
PARSEC_DECLSPEC void parsec_output_set_verbosity(int output_id, int level);
 ...
 */
PARSEC_DECLSPEC int parsec_output_get_verbosity(int output_id);
 ...
 * parsec_output_stream_t::lds_want_file is true, the output
 ...
 * after parsec_output_set_output_file_info() returns.
 *
 ...
 * to the new directory).  More specifically, the parsec_output
 ...
 */
PARSEC_DECLSPEC void parsec_output_set_output_file_info(const char *dir,
                                                      const char *prefix,
                                                      char **olddir,
                                                      char **oldprefix);
 ...
 * will be "compiled out" when PARSEC is configured without
 * PARSEC_DEBUG_NOISIER.
 *
 * @see parsec_output()
 */
#define PARSEC_OUTPUT(a) parsec_output a

/**
 * Macro for use in sending debugging output to the output
 * streams.  Will be "compiled out" when PARSEC is configured
 * without PARSEC_DEBUG_NOISIER.
 *
 * @see parsec_output_verbose()
 */
#define PARSEC_OUTPUT_VERBOSE(a) parsec_output_verbose a
#else
/**
 * Main macro for use in sending debugging output to output streams;
 * will be "compiled out" when PARSEC is configured without
 ...
 * @see parsec_output_verbose()
 */
#define PARSEC_OUTPUT_VERBOSE(a)
#endif
