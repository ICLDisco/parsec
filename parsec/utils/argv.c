 ...
 */
int parsec_argv_append(int *argc, char ***argv, const char *arg)
{
    int rc;

    /* add the new element */
    if (PARSEC_SUCCESS != (rc = parsec_argv_append_nosize(argv, arg))) {
        return rc;
    }
...
int parsec_argv_append_nosize(char ***argv, const char *arg)
{
    int argc;
...
int parsec_argv_prepend_nosize(char ***argv, const char *arg)
{
    int argc;
    int i;
...
int parsec_argv_append_unique_nosize(char ***argv, const char *arg, bool overwrite)
{
    int i;
     ...
     */
    if (NULL == *argv) {
        return parsec_argv_append_nosize(argv, arg);
    }
 ...
 */
void parsec_argv_free(char **argv)
{
  char **p;
 ...
 */
static char **parsec_argv_split_inter(const char *src_string, int delimiter,
        int include_empty)
{
  char arg[ARGSIZE];
  char **argv = NULL;
  ...
  while (src_string && *src_string) {
    ...
    if (src_string == p) {
      if (include_empty) {
        arg[0] = '\0';
        if (PARSEC_SUCCESS != parsec_argv_append(&argc, &argv, arg))
          return NULL;
      }
    }
    ...
    /* tail argument, add straight from the original string */
      ...
      if (PARSEC_SUCCESS != parsec_argv_append(&argc, &argv, src_string))
        return NULL;
      src_string = p;
      continue;
    }
    ...
    /* long argument, malloc buffer, copy and add */
      ...
      if (PARSEC_SUCCESS != parsec_argv_append(&argc, &argv, argtemp)) {
        free(argtemp);
        return NULL;
      }
...
char **parsec_argv_split(const char *src_string, int delimiter)
{
    return parsec_argv_split_inter(src_string, delimiter, 0);
}

char **parsec_argv_split_with_empty(const char *src_string, int delimiter)
{
    return parsec_argv_split_inter(src_string, delimiter, 1);
}

/*
 * Return the length of a NULL-terminated argv array.
 */
int parsec_argv_count(char **argv)
{
  char **p;
  int i;
 ...
 */
char *parsec_argv_join(char **argv, int delimiter)
{
  char **p;
  char *pp;
  char *str;
...
char *parsec_argv_join_range(char **argv, size_t start, size_t end, int delimiter)
{
    ...
    if (NULL == argv || NULL == argv[0] || (int)start > parsec_argv_count(argv)) {
        return strdup("");
    }
 ...
 */
size_t parsec_argv_len(char **argv)
{
  char **p;
  size_t length;
 ...
 */
char **parsec_argv_copy(char **argv)
{
  char **dupv = NULL;
  int dupc = 0;
  ...
  while (NULL != *argv) {
    if (PARSEC_SUCCESS != parsec_argv_append(&dupc, &dupv, *argv)) {
      parsec_argv_free(dupv);
      return NULL;
    }
...
int parsec_argv_delete(int *argc, char ***argv, int start, int num_to_delete)
{
    int i;
    int count;
    int suffix_count;
...
int parsec_argv_insert(char ***target, int start, char **source)
{
    ...
    target_count = parsec_argv_count(*target);
    source_count = parsec_argv_count(source);
    if (start > target_count) {
        for (i = 0; i < source_count; ++i) {
            parsec_argv_append(&target_count, target, source[i]);
        }
    }
...
int parsec_argv_insert_element(char ***target, int location, char *source)
{
    int i, target_count;
    int suffix_count;
    ...
    if (location > target_count) {
        parsec_argv_append(&target_count, target, source);
        return PARSEC_SUCCESS;
    }
