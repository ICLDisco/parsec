 ...
 *
 * The cpusets stored in the VP map are expressed as PaRSEC binding-resource
 * indexes. They describe the default candidate resources for each thread;
...
{
    return parsec_nb_total_threads;
}
...
build_flat_topology:
    ...
    for( int i = 0; i < parsec_nbvp; i++ ) {
        ...
        for( int j = 0; j < parsec_vpmap[i].nbthreads; j++ ) {
            if( NULL == parsec_vpmap[i].threads[j].cpuset ) {
    ...
    for( vp_id = 0; vp_id < parsec_nbvp; vp_id++ ) {
        ...
        for( th_id = 0; th_id < nbthreadspervp; th_id += parsec_nbht ) {
            for( ht_id = 0; ht_id < parsec_nbht ; ht_id++ ) {
                ...
                parsec_vpmap[vp_id].threads[th_id + ht_id].cpuset = HWLOC_ALLOC();
                HWLOC_SET(parsec_vpmap[vp_id].threads[th_id + ht_id].cpuset, core_id);
