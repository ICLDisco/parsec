...
int
parsec_default_dpcpp_stage_in(parsec_gpu_task_t        *gtask,
                              uint32_t                  flow_mask,
                              parsec_gpu_exec_stream_t *gpu_stream)
{
    return parsec_default_level_zero_stage_in(gtask, flow_mask, gpu_stream);
}
...
int
parsec_default_dpcpp_stage_out(parsec_gpu_task_t        *gtask,
                               uint32_t                  flow_mask,
                               parsec_gpu_exec_stream_t *gpu_stream)
{
    return parsec_default_level_zero_stage_out(gtask, flow_mask, gpu_stream);
}
    ...
    *out_task = NULL;
    ...
    if( NULL != stream->tasks[stream->end] ) {
        ...
        if( ZE_RESULT_SUCCESS == rc ) {
            ...
            rc = zeCommandListReset(level_zero_stream->command_lists[stream->end]);
            PARSEC_LEVEL_ZERO_CHECK_ERROR( "zeCommandListReset ", rc, {} );
...
parsec_hook_return_t
parsec_dpcpp_kernel_scheduler( parsec_execution_stream_t *es,
                               parsec_gpu_task_t         *gpu_task,
                               int which_gpu )
{
    return parsec_level_zero_kernel_scheduler(es, gpu_task, which_gpu);
}
