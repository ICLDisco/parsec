...
struct dague_handle_s {
    ...
    uint32_t                   handle_id;
    volatile uint32_t          nb_tasks;  /**< The number of tasks as viewed by the upper level support. This
                                           *   field is dedicated to the upper level and has only a loose meaning
                                           *   as long as it is not zero. Once it reaches zero all local tasks are
                                           *   considered completed, and the number of pending actions will be
                                           *   decreased by one to signal the upper level is now done with this handle.
                                           */
    uint16_t                   nb_functions;
    uint16_t                   devices_mask;
    int32_t                    priority;
    volatile uint32_t          nb_pending_actions;  /**< Internal counter of pending actions dedicated to track
                                                     *   all runtime activities (such as communications, data
                                                     *   movement, and so on). It also count one element for all the
                                                     *   tasks generated by the DSL.
                                                     */
