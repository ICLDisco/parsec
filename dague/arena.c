...
void dague_arena_destruct(dague_arena_t* arena)
{
    ...
    while(NULL != (item = dague_lifo_pop(&arena->area_lifo))) {
        DEBUGVV("Arena:\tfree element base ptr %p, data ptr %p (from arena %p)\n",
                          ...
                          dague_arena_chunk_t *chunk)
{
    ...
    if( (chunk->count == 1) && (arena->released < arena->max_released) ) {
        DEBUGV("Arena:\tpush a data of size %zu from arena %p, aligned by %zu, base ptr %p, data ptr %p, sizeof prefix %zu(%zd)\n",
    ...
    DEBUGV("Arena:\tdeallocate a tile of size %zu x %zu from arena %p, aligned by %zu, base ptr %p, data ptr %p, sizeof prefix %zu(%zd)\n",
