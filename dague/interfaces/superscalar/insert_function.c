...
/* Dependencies settting function for Function structures */
void 
 ...
 * Each time the ser calls it a task is created with the respective parameters the user has passed.
 * For each task class a structure known as "function" is created as well. (e.g. for Cholesky 4 function strucutres are created for each task class).
                      ...
                      char* name)
{
    ...
    if( NULL == tmp_function ) {
        dague_function_t *function = create_function(__dague_handle, fpointer, name);
    } else {
        
    }
    
    temp_task->belongs_to_function = 
    (find_function(__dague_handle->function_h_table, fpointer, __dague_handle->function_hash_table_size))->function_id;
