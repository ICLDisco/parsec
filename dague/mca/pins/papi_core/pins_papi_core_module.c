...
static void pins_thread_init_papi_core(dague_execution_unit_t * exec_unit) {
    ...
    event_cb->num_core_counters = 0;
    event_cb->pins_papi_core_event_name = (char**)calloc(NUM_CORE_EVENTS, sizeof(char*));
    event_cb->pins_papi_core_native_event = (int*)calloc(NUM_CORE_EVENTS, sizeof(int));
    event_cb->papi_eventset = PAPI_NULL;

    for(i = 0; i < NUM_CORE_EVENTS; i++) {
        event_cb->pins_papi_core_event_name[i] = NULL;
        event_cb->pins_papi_core_native_event[i] = PAPI_NULL;
    }
    ...
    while(token != NULL) {
        ...
        if(socket && core) {
            if(event_cb->num_core_counters == NUM_CORE_EVENTS) {
            ...
            if(PAPI_OK != PAPI_event_name_to_code(token, &event_cb->pins_papi_core_native_event[event_cb->num_core_counters]) )
                break;
    ...
    if(event_cb->num_core_counters > 0) {
        char* key_string;
        char* value_string;
        int string_size = 0;
        ...
        dague_profiling_add_dictionary_keyword(key_string, "fill:#00AAFF",
                                               sizeof(uint64_t) * event_cb->num_core_counters, value_string,
                                               &event_cb->pins_papi_core[0],
                                               &event_cb->pins_papi_core[1]);
        free(key_string);
        free(value_string);
                            ...
                            parsec_pins_next_callback_t* cb_data)
{
    ...
    (void)dague_profiling_trace(exec_unit->eu_profile, event_cb->pins_papi_core[0],
                                45, 0, (void *)&info);
}
                           ...
                           parsec_pins_next_callback_t* cb_data)
{
    ...
    (void)dague_profiling_trace(exec_unit->eu_profile, event_cb->pins_papi_core[1],
                                45, 0, (void *)&info);
}
