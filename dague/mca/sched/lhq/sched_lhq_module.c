    ...
    for(p = 0; p < master->nb_vp; p++) {
        ...
        for(t = 0; t < vp->nb_cores; t++) {
            ...
            for(int level = 0; level < sched_obj->nb_hierarch_queues; level++) {
                ...
                if( eu->th_id == m ) {
                    ...
                    DAGUE_DEBUG_VERBOSE(20, dague_debug_output, "schedHQ %d: \tcreates hbbuffer of size %d (ideal %d) for level %d stored in %d: %p (parent: %p -- %s)\n",
        ...
        for(t = 0; t < vp->nb_cores; t++) {
            ...
            for(int level = 0; level < sched_obj->nb_hierarch_queues; level++) {
                ...
                int m = dague_hwloc_master_id(level, eu->th_id);
                if( eu->th_id != m ) {
                    DAGUE_DEBUG_VERBOSE(20, dague_debug_output, "schedHQ %d: \ttakes the buffer of %d at level %d stored in %d: %p\n",
