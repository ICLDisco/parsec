...
static dague_output_stream_t verbose;
 ...
 * Private functions
 */
static void construct(dague_object_t *stream);
...
/* global state */
bool dague_output_redirected_to_syslog = false;
int dague_output_redirected_syslog_pri;
...
static output_desc_t info[DAGUE_OUTPUT_MAX_STREAMS];
...
bool dague_output_init(void)
{
    ...
    str = getenv("DAGUE_OUTPUT_STDERR_FD");
    if (NULL != str) {
        default_stderr_fd = atoi(str);
    }
    str = getenv("DAGUE_OUTPUT_REDIRECT");
    if (NULL != str) {
        if (0 == strcasecmp(str, "syslog")) {
            dague_output_redirected_to_syslog = true;
        }
    }
    str = getenv("DAGUE_OUTPUT_SYSLOG_PRI");
    if (NULL != str) {
        if (0 == strcasecmp(str, "info")) {
            dague_output_redirected_syslog_pri = LOG_INFO;
        } else if (0 == strcasecmp(str, "error")) {
            dague_output_redirected_syslog_pri = LOG_ERR;
        } else if (0 == strcasecmp(str, "warn")) {
            dague_output_redirected_syslog_pri = LOG_WARNING;
        } else {
            dague_output_redirected_syslog_pri = LOG_ERR;
        }
    } else {
    ...
    str = getenv("DAGUE_OUTPUT_SYSLOG_IDENT");
    if (NULL != str) {
        redirect_syslog_ident = strdup(str);
    }

    OBJ_CONSTRUCT(&verbose, dague_output_stream_t);
    ...
    if (dague_output_redirected_to_syslog) {
        ...
        verbose.lds_syslog_priority = dague_output_redirected_syslog_pri;
        if (NULL != str) {
    ...
    for (i = 0; i < DAGUE_OUTPUT_MAX_STREAMS; ++i) {
        ...
        info[i].ldi_syslog = dague_output_redirected_to_syslog;
    ...
    rc = asprintf(&output_prefix, "output-pid%d-", getpid());
 ...
 * Open a stream
 */
int dague_output_open(dague_output_stream_t * lds)
{
 ...
 */
int dague_output_reopen(int output_id, dague_output_stream_t * lds)
{
 ...
 */
bool dague_output_switch(int output_id, bool enable)
{
    bool ret = false;
 ...
 * Close a stream
 */
void dague_output_close(int output_id)
{
    int i;
 ...
 */
void dague_output(int output_id, const char *format, ...)
{
    if (output_id >= 0 && output_id < DAGUE_OUTPUT_MAX_STREAMS) {
        va_list arglist;
 ...
 */
void dague_output_verbose(int level, int output_id, const char *format, ...)
{
    if (output_id >= 0 && output_id < DAGUE_OUTPUT_MAX_STREAMS &&
 ...
 */
char *dague_output_string(int level, int output_id, const char *format, ...)
{
    int rc;
    char *ret = NULL;
 ...
 */
void dague_output_set_verbosity(int output_id, int level)
{
    if (output_id >= 0 && output_id < DAGUE_OUTPUT_MAX_STREAMS) {
        info[output_id].ldi_verbose_level = level;
    }
 ...
 */
void dague_output_set_output_file_info(const char *dir,
                                      const char *prefix,
                                      char **olddir,
                                      char **oldprefix)
{
 ...
 */
void dague_output_finalize(void)
{
    if (initialized) {
        if (verbose_stream != -1) {
            dague_output_close(verbose_stream);
        }
        free(verbose.lds_prefix);
        verbose_stream = -1;
 ...
 * Constructor
 */
static void construct(dague_object_t *obj)
{
    dague_output_stream_t *stream = (dague_output_stream_t*) obj;
...
static int output(int output_id, const char *format, va_list arglist)
{
    ...
    if (output_id >= 0 && output_id < DAGUE_OUTPUT_MAX_STREAMS &&
        ...
        /* Make the strings */
        if (DAGUE_SUCCESS != (rc = make_string(&str, ldi, format, arglist))) {
            dague_atomic_unlock(&mutex);
            return rc;
        }
         ...
         * dague_output.h docs about this!). */

        if (ldi->ldi_file) {
            if (ldi->ldi_fd == -1) {
                if (DAGUE_SUCCESS != open_file(output_id)) {
...
int dague_output_get_verbosity(int output_id)
{
    if (output_id >= 0 && output_id < DAGUE_OUTPUT_MAX_STREAMS && info[output_id].ldi_used) {
