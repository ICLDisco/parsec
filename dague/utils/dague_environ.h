 ...
 * later be freed with a call to dague_argv_free().
 *
 ...
 */
DAGUE_DECLSPEC char **dague_environ_merge(char **minor, char **major);
 ...
 *
 * \code
 *   #include "opal/util/dague_environ.h"
 *   dague_setenv("foo", "bar", true, &environ);
 * \endcode
 *
 * NOTE: If you use the real environ, dague_setenv() will turn
 ...
 * \code
 *   char **my_env = NULL;
 *   dague_setenv("foo", "bar", true, &my_env);
 * \endcode
 */
DAGUE_DECLSPEC int dague_setenv(const char *name, const char *value,
 ...
 */
DAGUE_DECLSPEC int dague_unsetenv(const char *name, char ***env);

/* A consistent way to retrieve the home and tmp directory on all supported
 * platforms.
 */
DAGUE_DECLSPEC const char* dague_home_directory( void );
DAGUE_DECLSPEC const char* dague_tmp_directory( void );
