...
int dague_argv_prepend_nosize(char ***argv, const char *arg)
{
    int argc;
    int i;
 ...
 */
void dague_argv_free(char **argv)
{
  char **p;
 ...
 */
static char **dague_argv_split_inter(const char *src_string, int delimiter,
        int include_empty)
{
  char arg[ARGSIZE];
  char **argv = NULL;
...
char **dague_argv_split(const char *src_string, int delimiter)
{
    return dague_argv_split_inter(src_string, delimiter, 0);
}

char **dague_argv_split_with_empty(const char *src_string, int delimiter)
{
    return dague_argv_split_inter(src_string, delimiter, 1);
}

/*
 * Return the length of a NULL-terminated argv array.
 */
int dague_argv_count(char **argv)
{
  char **p;
  int i;
 ...
 */
char *dague_argv_join(char **argv, int delimiter)
{
  char **p;
  char *pp;
  char *str;
 ...
 */
char *dague_argv_join_range(char **argv, size_t start, size_t end, int delimiter)
{
    char **p;
    char *pp;
    char *str;
 ...
 */
size_t dague_argv_len(char **argv)
{
  char **p;
  size_t length;
 ...
 */
char **dague_argv_copy(char **argv)
{
  char **dupv = NULL;
  int dupc = 0;
  ...
  while (NULL != *argv) {
    if (DAGUE_SUCCESS != dague_argv_append(&dupc, &dupv, *argv)) {
      dague_argv_free(dupv);
      return NULL;
    }
...
int dague_argv_delete(int *argc, char ***argv, int start, int num_to_delete)
{
    int i;
    int count;
    int suffix_count;
...
int dague_argv_insert(char ***target, int start, char **source)
{
    ...
    source_count = dague_argv_count(source);
    if (start > target_count) {
        for (i = 0; i < source_count; ++i) {
            dague_argv_append(&target_count, target, source[i]);
        }
    }
...
int dague_argv_insert_element(char ***target, int location, char *source)
{
    int i, target_count;
    int suffix_count;
