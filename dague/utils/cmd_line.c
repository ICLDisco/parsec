...
struct cmd_line_option_t {
    ...
    dague_cmd_line_type_t clo_type;
...
/*
 * Instantiate the dague_cmd_line_t class
 */
static void cmd_line_constructor(dague_cmd_line_t *cmd);
static void cmd_line_destructor(dague_cmd_line_t *cmd);
OBJ_CLASS_INSTANCE(dague_cmd_line_t,
                   dague_object_t,
 ...
 * Private functions
 */
static int make_opt(dague_cmd_line_t *cmd, dague_cmd_line_init_t *e);
static void free_parse_results(dague_cmd_line_t *cmd);
static int split_shorts(dague_cmd_line_t *cmd,
                        char *token, char **args,
                        int *output_argc, char ***output_argv,
                        int *num_args_used, bool ignore_unknown);
static cmd_line_option_t *find_option(dague_cmd_line_t *cmd,
 ...
 */
int dague_cmd_line_create(dague_cmd_line_t *cmd,
                         dague_cmd_line_init_t *table)
{
    int i, ret = DAGUE_SUCCESS;
    ...
    OBJ_CONSTRUCT(cmd, dague_cmd_line_t);
 ...
 */
int dague_cmd_line_make_opt_mca(dague_cmd_line_t *cmd,
 ...
 */
int dague_cmd_line_make_opt3(dague_cmd_line_t *cmd, char short_name,
                            const char *sd_name, const char *long_name,
                            int num_params, const char *desc)
{
    dague_cmd_line_init_t e;
    ...
    e.ocl_variable_type = DAGUE_CMD_LINE_TYPE_NULL;
 ...
 * handle.
 */
int dague_cmd_line_parse(dague_cmd_line_t *cmd, bool ignore_unknown,
                        int argc, char **argv)
{
    int i, j, orig, ret;
    ...
    dague_atomic_lock(&cmd->lcl_mutex);
    ...
    cmd->lcl_argc = argc;
    cmd->lcl_argv = dague_argv_copy(argv);
    ...
    for (i = 1; i < cmd->lcl_argc; ) {
        ...
        if (0 == strcmp(cmd->lcl_argv[i], "--")) {
            ...
            while (i < cmd->lcl_argc) {
                dague_argv_append(&cmd->lcl_tail_argc, &cmd->lcl_tail_argv,
                                 cmd->lcl_argv[i]);
                ++i;
            }
        ...
        /* It could be a short name.  Is it? */
            ...
            if (NULL == option) {
                ...
                if (DAGUE_SUCCESS == ret) {
                    ...
                    if (NULL != option) {
                        dague_argv_delete(&cmd->lcl_argc,
			                 &cmd->lcl_argv, i,
                                         1 + num_args_used);
                        dague_argv_insert(&cmd->lcl_argv, i, shortsv);
                        cmd->lcl_argc = dague_argv_count(cmd->lcl_argv);
                    } else {
                        is_unknown_option = true;
                    }
                    dague_argv_free(shortsv);
                } else {
                    is_unknown_option = true;
                }
            }
        ...
        if (is_option) {
            if (NULL == option) {
                ...
                if (NULL == param) {
                    dague_atomic_unlock(&cmd->lcl_mutex);
                    ...
                    if (i >= cmd->lcl_argc) {
                        ...
                        if (0 == strcmp(cmd->lcl_argv[i],
                            ...
                            if (NULL != param->clp_argv) {
                                dague_argv_free(param->clp_argv);
                            }
                            OBJ_RELEASE(param);
                            printed_error = true;
                            goto error;
                        ...
                        /* Otherwise, save this parameter */
                            ...
                            dague_argv_append(&param->clp_argc,
                                             &param->clp_argv,
                                             cmd->lcl_argv[i]);
                            ...
                            if (0 == j &&
                                ...
                                if (DAGUE_SUCCESS != (ret = set_dest(option, cmd->lcl_argv[i]))) {
                                    dague_atomic_unlock(&cmd->lcl_mutex);
                                    return ret;
                                }
                            }
                ...
                if (0 == option->clo_num_params) {
                    if (DAGUE_SUCCESS != (ret = set_dest(option, "1"))) {
                        dague_atomic_unlock(&cmd->lcl_mutex);
                        return ret;
                    }
                }
                ...
                /* If we succeeded in all that, save the param to the
                   list on the dague_cmd_line_t handle */

                if (NULL != param) {
                    dague_list_append(&cmd->lcl_params, &param->super);
                }
            }
        }
 ...
 */
char *dague_cmd_line_get_usage_msg(dague_cmd_line_t *cmd)
{
    size_t i, len;
    int argc, count, j;
    char **argv;
    ...
    for (count = 0, item = DAGUE_LIST_ITERATOR_FIRST(&cmd->lcl_options);
         DAGUE_LIST_ITERATOR_END(&cmd->lcl_options) != item;
         ++count, item = DAGUE_LIST_ITERATOR_NEXT(item)) {
    }
    ...
    for (i = 0, item = DAGUE_LIST_ITERATOR_FIRST(&cmd->lcl_options);
         DAGUE_LIST_ITERATOR_END(&cmd->lcl_options) != item;
         ++i, item = DAGUE_LIST_ITERATOR_NEXT(item)) {
 ...
 */
bool dague_cmd_line_is_taken(dague_cmd_line_t *cmd, const char *opt)
{
    return (dague_cmd_line_get_ninsts(cmd, opt) > 0);
}
 ...
 */
int dague_cmd_line_get_ninsts(dague_cmd_line_t *cmd, const char *opt)
{
    int ret;
    dague_list_item_t *item;
    cmd_line_param_t *param;
    ...
    if (NULL != option) {
        for (item = DAGUE_LIST_ITERATOR_FIRST(&cmd->lcl_params);
             DAGUE_LIST_ITERATOR_END(&cmd->lcl_params) != item;
 ...
 */
char *dague_cmd_line_get_param(dague_cmd_line_t *cmd, const char *opt, int inst,
                              int idx)
{
    int num_found;
    dague_list_item_t *item;
 ...
 */
int dague_cmd_line_get_argc(dague_cmd_line_t *cmd)
{
    return (NULL != cmd) ? cmd->lcl_argc : DAGUE_ERROR;
}
 ...
 */
char *dague_cmd_line_get_argv(dague_cmd_line_t *cmd, int index)
{
 ...
 * line handle.
 */
int dague_cmd_line_get_tail(dague_cmd_line_t *cmd, int *tailc, char ***tailv)
{
    if (NULL != cmd) {
        dague_atomic_lock(&cmd->lcl_mutex);
        *tailc = cmd->lcl_tail_argc;
        *tailv = dague_argv_copy(cmd->lcl_tail_argv);
        dague_atomic_unlock(&cmd->lcl_mutex);
        return DAGUE_SUCCESS;
    } else {
        return DAGUE_ERROR;
...
static void option_constructor(cmd_line_option_t *o)
{
    ...
    o->clo_type = DAGUE_CMD_LINE_TYPE_NULL;
...
static void cmd_line_constructor(dague_cmd_line_t *cmd)
{
    ...
    OBJ_CONSTRUCT(&cmd->lcl_options, dague_list_t);
    OBJ_CONSTRUCT(&cmd->lcl_params, dague_list_t);
...
static void cmd_line_destructor(dague_cmd_line_t *cmd)
{
    ...
    for (item = dague_list_pop_front(&cmd->lcl_options);
         NULL != item;
         item = dague_list_pop_front(&cmd->lcl_options)) {
        OBJ_RELEASE(item);
    }
...
static int make_opt(dague_cmd_line_t *cmd, dague_cmd_line_init_t *e)
{
    ...
    dague_atomic_lock(&cmd->lcl_mutex);
    dague_list_append(&cmd->lcl_options, &option->super);
    dague_atomic_unlock(&cmd->lcl_mutex);
...
static void free_parse_results(dague_cmd_line_t *cmd)
{
    ...
    for (item = dague_list_pop_front(&cmd->lcl_params);
         NULL != item;
         item = dague_list_pop_front(&cmd->lcl_params)) {
        OBJ_RELEASE(item);
    }
    ...
    if (NULL != cmd->lcl_argv) {
        dague_argv_free(cmd->lcl_argv);
    }
    cmd->lcl_argv = NULL;
    cmd->lcl_argc = 0;

    if (NULL != cmd->lcl_tail_argv) {
        dague_argv_free(cmd->lcl_tail_argv);
    }
    cmd->lcl_tail_argv = NULL;
    cmd->lcl_tail_argc = 0;
}
 ...
 * "single dash" names.
 */
static int split_shorts(dague_cmd_line_t *cmd, char *token, char **args,
                        int *output_argc, char ***output_argv,
                        int *num_args_used, bool ignore_unknown)
{
    int i, j, len;
    ...
    num_args = dague_argv_count(args);
    *num_args_used = 0;
    ...
    for (i = 0; i < len; ++i) {
        ...
        if (NULL == option) {
            if (!ignore_unknown) {
                ...
                dague_argv_append(output_argc, output_argv, fake_token);
            }
        }
           ...
           handled at a higher level) */
            ...
            for (j = 0; j < option->clo_num_params; ++j) {
                if (*num_args_used < num_args) {
                    dague_argv_append(output_argc, output_argv,
                                     args[*num_args_used]);
                    ++(*num_args_used);
                } else {
                    dague_argv_append(output_argc, output_argv,
                                      ...
                                      const char *option_name)
{
    ...
    /* Iterate through the list of options hanging off the
       dague_cmd_line_t and see if we find a match in either the short
       or long names */

    for (item = DAGUE_LIST_ITERATOR_FIRST(&cmd->lcl_options);
         DAGUE_LIST_ITERATOR_END(&cmd->lcl_options) != item;
         item = DAGUE_LIST_ITERATOR_NEXT(item)) {
