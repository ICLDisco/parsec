...
#elif defined(DAGUE_HAVE_SCHED_SETAFFINITY)
...
#elif defined(DAGUE_HAVE_HWLOC)
    {
        cpu = dague_hwloc_bind_on_core_index(cpu, ht);
        if(cpu == -1 ) {
            dague_warning("Core binding on node %i failed", cpu);
            return -1;
        }
    }
...
#ifdef DAGUE_HAVE_OLD_SCHED_SETAFFINITY
        if(sched_setaffinity(0,&mask) < 0)
#else /* DAGUE_HAVE_OLD_SCHED_SETAFFINITY */
        if(sched_setaffinity(0,sizeof(mask),&mask) < 0)
#endif /* DAGUE_HAVE_OLD_SCHED_SETAFFINITY */
            {
                return -1;
            }
    }
