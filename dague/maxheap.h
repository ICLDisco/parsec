...
heap_split_and_steal(dague_heap_t ** heap_ptr,
                     dague_heap_t ** new_heap_ptr);
...
dague_execution_context_t * heap_split_and_steal(dague_heap_t ** heap_ptr, dague_heap_t ** new_heap_ptr)
{
    ...
    if (heap != NULL) {
        ...
        if (heap->top->super.list_item.list_prev == NULL) {
            ...
            DAGUE_DEBUG_VERBOSE(20, dague_debug_output, "MH:\tDestroying heap %p\n", heap->top, heap->top->super.list_item.list_next, heap);
            heap->top = NULL;
            ...
            if (heap->top->super.list_item.list_next /* right */ == NULL) {
                ...
                DAGUE_DEBUG_VERBOSE(20, dague_debug_output, "MH:\tSplit heap %p into itself and heap %p\n", heap, *new_heap_ptr);
            }
        }
    ...
    if (to_use != NULL) {
        char tmp[MAX_TASK_STRLEN];
        DAGUE_DEBUG_VERBOSE(20, dague_debug_output, "MH:\tStole exec C %s (%p) from heap %p\n", dague_snprintf_execution_context(tmp, MAX_TASK_STRLEN, to_use), to_use, heap);
    }
#endif
    return to_use;
}
