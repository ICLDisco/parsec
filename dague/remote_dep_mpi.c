                         ...
                         dague_remote_deps_t* origin)
{
    ...
    for(k = 0; origin->msg.output_mask>>k; k++) {
        ...
        DAGUE_DEBUG_VERBOSE(20, dague_debug_output, "MPI:\tRetrieve datatype with mask 0x%x (remote_dep_get_datatypes)", local_mask);
...
static inline int next_tag(int k) {
    ...
    if( __tag > (MAX_MPI_TAG-k) ) {
        DAGUE_DEBUG_VERBOSE(20, dague_debug_output, "rank %d tag rollover: min %d < %d (+%d) < max %d", dague_debug_rank,
                                   ...
                                   int* position)
{
    ...
    if( (length - (*position)) < dsize ) {  /* no room. bail out */
        DAGUE_DEBUG_VERBOSE(20, dague_debug_output, "Can't pack at %d/%d. Bail out!", *position, length);
        return 1;
    }
    ...
    *position += dsize;
    ...
    for(k = 0; deps->outgoing_mask >> k; k++) {
        ...
        if(NULL == deps->output[k].data.arena) {
            DAGUE_DEBUG_VERBOSE(10, dague_debug_output, " CTL\t%s\tparam %d\tdemoted to be a control", tmp, k);
            continue;
        }
                         ...
                         dep_cmd_item_t* item)
{
    ...
    DAGUE_DEBUG_VERBOSE(20, dague_debug_output, "MPI:\tPUT mask=%lx deps 0x%lx", task->output_mask, task->deps);
