...
int main(int argc, char **argv) {
	...
	int descA[9], descB[9];
	float *A=NULL, *B=NULL, *Acpy=NULL, *X=NULL, *work=NULL;
	float XnormF, AnormF, RnormF, residF;
/**/
...
/**/
	...
	A = (float *)malloc(mloc*nloc*sizeof(float)) ;

	iseed = iam*mloc*nloc; srand(iseed);
	k = 0;
	for (i = 0; i < mloc; i++) {
		for (j = 0; j < nloc; j++) {
			A[k] = ((float) rand()) / ((float) RAND_MAX) - 0.5f;
			k++;	
		}
	}
	...
	for (i = 1; i <= n; i++) {
		...
		&&   ( mycol == indxg2p_( &i, &nb, &idum1, &i0, &npcol ) ) ){
			...
			A[ (jloc-1)*mloc + (iloc-1) ] += ((float) n);
		}
		
	}

	if (verif==1){
		Acpy = (float *)malloc(mloc*nloc*sizeof(float)) ;
      		{ int i1=1; pslacpy_( "A", &n, &n, A, &i1, &i1, descA, Acpy, &i1, &i1, descA ); }
	}


	my_elapsed =- MPI_Wtime();
	{ int i1=1; pspotrf_( "L", &n, A, &i1, &i1, descA, &my_info_facto ); }
	my_elapsed += MPI_Wtime();
	...
	if (verif==1){
		...
		B = (float *)malloc(mloc*sloc*sizeof(float)) ;
		k = 0;
		for (i = 0; i < mloc; i++) {
			for (j = 0; j < sloc; j++) {
				B[k] = ((float) rand()) / ((float) RAND_MAX) - 0.5f;
				k++;	
			}
		}

		X = (float *)malloc(mloc*sloc*sizeof(float)) ;
      		{ int i1=1; pslacpy_( "A", &n, &s, B, &i1, &i1, descB, X, &i1, &i1, descB ); }

		{ int i1=1; pspotrs_( "L", &n, &s, A, &i1, &i1, descA, X, &i1, &i1, descB, &my_info_solve ); }
		MPI_Allreduce( &my_info_solve, &info_solve, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);

		{ int i1=1; float pone=1.00e+00f, mone = -1.00e+00f; pssymm_( "L", "L", &n, &s, &mone, Acpy, &i1, &i1, descA, X, &i1, &i1, descB,
			&pone, B, &i1, &i1, descB); }
		{ int i1=1; AnormF = pslansy_( "F", "L", &n, Acpy, &i1, &i1, descA, work); }
		{ int i1=1; XnormF = pslange_( "F", &n, &s, X, &i1, &i1, descB, work); }
		{ int i1=1; RnormF = pslange_( "F", &n, &s, B, &i1, &i1, descB, work); }
