...
int main(int argc, char **argv) {
	...
	int i, j, k, info_pdgetrf, info_pdpotrs, info, iseed, verif;
	int my_info_pdgetrf, my_info_pdpotrs;
    int *ippiv;
	int descA[9], descB[9];
...
/**/
	...
	{ int i1=1; pdgetrf_( &n, &n, A, &i1, &i1, descA, ippiv, &my_info_pdgetrf ); }
	my_elapsed += MPI_Wtime();

	MPI_Allreduce( &my_elapsed, &elapsed, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
	MPI_Allreduce( &my_info_pdgetrf, &info_pdgetrf, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
	...
	if (0 /*verif==1*/){
		...
		if ( iam == 0 ){
			...
			printf("SCAL GETRF            %6d %3d %4d %4d %3d %3d %6.2f %6.2lf %6.2e %d %d\n", n, s, nb, nprocs, nprow, npcol, elapsed, GFLOPS, residF, info_pdgetrf, info_pdpotrs);
		}
