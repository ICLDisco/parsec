...
int main(int argc, char **argv) {
	...
	descinit_( descA, &n, &n, &nb, &nb, &izero, &izero, &ictxt, &mloc, &info );
	...
	if ( iam==0 ){
		printf("\n\tnrhs\tnb\tp\tq\tinfo\tresid\ttime(s)  \tGFLOPS/sec\tGFLOPS/sec/proc\n");
		if( do_validation ) {
			printf("%d\t%d\t%d\t%d\t%d\t%d\t%1.1f\t%f\t%f\t%f\n\n",
				n, nrhs, nb, nprow, npcol, info, residF, MPIelapsed, GFLOPS, GFLOPS_per_proc);
		} else {
			printf("%d\t%d\t%d\t%d\t%d\t%d\tx.x\t%f\t%f\t%f\n\n",
				n, nrhs, nb, nprow, npcol, info, MPIelapsed, GFLOPS, GFLOPS_per_proc);
		}
	}
