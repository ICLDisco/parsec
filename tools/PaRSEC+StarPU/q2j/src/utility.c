...
#define STARPU_FIRST_VAR 1
//#define STARPU_ELEMS_PER_LINE 
...
static parsec_linked_list_t _parsec_pool_list;
...
static StarPU_fun_decl *starpu_find_fun_decl(char *name)
{
    StarPU_translation_list *tmp;
    for(tmp = trans_list; tmp != NULL; tmp = tmp->prev)
    {
	if(strcmp(tmp->tr->name, name) == 0)
	    return tmp->tr;
    }
...
char *starpu_symname(uint64_t pos, StarPU_task *task)
{
    uint64_t tmp = 0;
    ...
    fprintf(stderr, "Unable to find the parameter #%" PRIu64 " in task %s\n", pos, task->name);
    exit(-1);
    return NULL;
}

void starpu_add_glob_cpu(StarPU_task *task, node_t *node)
{
    StarPU_variable_list *last = task->glob;
    for(int i = 2; i<node->u.kids.kid_count; i++)
    {	
	node_t *tmp = node->u.kids.kids[i];
	if(tmp->type == IDENTIFIER && last != NULL) 
	    last->new_name = strdup(tmp->u.var_name);
	else if(tmp->type == EXPR && 
		(tmp->u.kids.kids[1])->type == IDENTIFIER && last != NULL) 
	    last->new_name = strdup((tmp->u.kids.kids[1])->u.var_name);
	else
	{
	    fprintf(stderr, "Unable to parse argument %d of starpu_unpack_args of task %s\n", i-1, task->name);
	    exit(-1);
	}
	last = last->next;
    }
...
void starpu_add_glob_cuda(StarPU_task *task, node_t *node)
{
    StarPU_variable_list *last = task->glob;
    for(int i = 2; i<node->u.kids.kid_count; i++)
    {
        node_t *tmp = node->u.kids.kids[i];
        if(tmp->type == IDENTIFIER && last != NULL)
            last->cuda_name = strdup(tmp->u.var_name);
        else if(tmp->type == EXPR &&
                (tmp->u.kids.kids[1])->type == IDENTIFIER && last != NULL)
            last->cuda_name = strdup((tmp->u.kids.kids[1])->u.var_name);
        else
        {
            fprintf(stderr, "Unable to parse argument %d of starpu_unpack_args of task %s\n", i-1, task->name);
            exit(-1);
...
void starpu_add_varname_cuda(StarPU_task *task, uint64_t pos, char *varname)
{
    StarPU_variable_list *vl;
    ...
    if(vl == NULL)
    {
	fprintf(stderr, "Unable to find the cuda's variable %s in the task %s\n", varname, task->name);
	exit(-1);
    }
    vl->cuda_name = strdup(varname);
}

void starpu_add_varname_cpu(StarPU_task *task, uint64_t pos, char *varname)
{
    StarPU_variable_list *vl;
    StarPU_variable_list *prec = NULL;

    for(vl = task->vl; vl != NULL; vl = vl->next)
    {
        if(vl->pos > pos && (prec == NULL || prec->pos < pos))
            break;
        prec = vl;
    }
    StarPU_variable_list *new_var = (StarPU_variable_list*) calloc(1, sizeof(struct StarPU_variable_list_t));
    new_var->new_name = strdup(varname);
    new_var->pos      = pos;
    new_var->next     = vl;
    if(prec == NULL)
	task->vl   = new_var;
...
void starpu_parse_cpu_function(node_t *node, StarPU_task *task)
{
    switch(node->type){
    case BLOCK:
        for(node_t *tmp = node->u.block.first; tmp != NULL; tmp = tmp->next)
            starpu_parse_cpu_function(tmp, task);
        break;
    case ASSIGN:
        if((node->u.kids.kids[1])->type == FCALL &&
           strcmp(((node->u.kids.kids[1])->u.kids.kids[0])->u.var_name, "STARPU_MATRIX_GET_PTR" ) == 0 &&
           (node->u.kids.kids[1])->u.kids.kid_count > 0)
        {
            node_t *tmp = (node->u.kids.kids[1])->u.kids.kids[1];
            if(tmp->type == ARRAY &&
               (tmp->u.kids.kids[1])->type == INTCONSTANT)
            {
		uint64_t num_buff = (tmp->u.kids.kids[1])->const_val.i64_value;
		starpu_add_varname_cpu(task, num_buff, (node->u.kids.kids[0])->u.var_name);
            }
            else
            {
                fprintf(stderr, "Bad expression in STARPU_MATRIX_GET_PTR\n");
            }
        }
    case FCALL:
	if(strcmp(node->u.kids.kids[0]->u.var_name, "starpu_codelet_unpack_args") == 0)
	    starpu_add_glob_cpu(task, node);
	break;
    default: break;
    }
}

void starpu_parse_cuda_function(node_t *node, StarPU_task *task)
{
    switch(node->type){
    case BLOCK:
        for(node_t *tmp = node->u.block.first; tmp != NULL; tmp = tmp->next)
	{ 
	    starpu_parse_cuda_function(tmp, task);
	}
        break;
    case ASSIGN:
        if((node->u.kids.kids[1])->type == FCALL &&
           ...
           (node->u.kids.kids[1])->u.kids.kid_count > 0)
	{
            ...
            {
                uint64_t num_buff = (tmp->u.kids.kids[1])->const_val.i64_value;
                starpu_add_varname_cuda(task, num_buff, (node->u.kids.kids[0])->u.var_name);
            }
            else
            {
                fprintf(stderr, "Bad expression in STARPU_MATRIX_GET_PTR\n");
    ...
    case FCALL:
        if(strcmp(node->u.kids.kids[0]->u.var_name, "starpu_codelet_unpack_args") == 0)
            starpu_add_glob_cuda(task, node);
        break;
    default: break;
    }
}

static void starpu_record_uses_defs_and_pools(node_t *node){
	...
	StarPU_task_list  *tl;
	StarPU_codelet    *cl;
	StarPU_param_list *pl;
	if(strcmp("starpu_insert_task", DA_kid(node,0)->u.var_name) ){
	    return;
	}
	tl = (StarPU_task_list*) calloc(1, sizeof(struct StarPU_task_list_t));
	tl->t = (StarPU_task*) calloc(1, sizeof(struct StarPU_task_t));
        kid_count = node->u.kids.kid_count;
        // StarPU specific code. The codelet is the first parameter.
        if( (kid_count > 1))
	{
	    if(EXPR == DA_kid(node,1)->type && 
	       DA_kid(node,1)->u.kids.kid_count == 2 &&
	       (DA_kid(node,1)->u.kids.kids[1])->type == IDENTIFIER)
	    {	// Codelet : &cl
		task = (task_t *)calloc(1, sizeof(task_t));
		task->task_name = strdup((DA_kid(node,1)->u.kids.kids[1])->u.var_name);
		tl->t->name = strdup((DA_kid(node,1)->u.kids.kids[1])->u.var_name);
		cl = starpu_call_to_codelet((DA_kid(node,1)->u.kids.kids[1])->u.var_name);
		task->task_node = node;
	    ...
	    else if(IDENTIFIER == DA_kid(node,1)->type)
	    {   // Codelet cl
                task = (task_t *)calloc(1, sizeof(task_t));
                task->task_name = strdup(DA_kid(node,1)->u.var_name);
		tl->t->name = strdup(DA_kid(node,1)->u.var_name);
		cl = starpu_call_to_codelet(DA_kid(node,1)->u.var_name);
		task->task_node = node;
		task->ind_vars = (char **)calloc(1+node->loop_depth, sizeof(char *));
                i=node->loop_depth-1;
                for(node_t *tmp=node->enclosing_loop; NULL != tmp; tmp=tmp->enclosing_loop ){
	    ...
	    StarPU_variable_list *tmp = tl->t->glob;
	    for(i=2; i<kid_count; i++)
	    {
		if(node->u.kids.kids[i]->type == IDENTIFIER && i+1<kid_count &&
		   strcmp((node->u.kids.kids[i])->u.var_name, "STARPU_VALUE") == 0)
		{
		    StarPU_variable_list *add = (StarPU_variable_list*) calloc(1, sizeof(struct StarPU_variable_list_t));
		    if(tmp == NULL)
			  tl->t->glob = add;
		    else  tmp->next   = add;
		    add->next = NULL;
		    
		    if((node->u.kids.kids[i+1])->type == EXPR &&
		       ((node->u.kids.kids[i+1])->u.kids.kids[1])->type == IDENTIFIER)
			add->call_name = strdup(((node->u.kids.kids[i+1])->u.kids.kids[1])->u.var_name);
		    else if ((node->u.kids.kids[i+1])->type == IDENTIFIER)
			add->call_name = strdup((node->u.kids.kids[i+1])->u.var_name);
		    add->pos = i;
		    tmp = add;
		}
	    }
    
	    for(pl = cl->l; pl != NULL; pl = pl->next)
	    {
		if(pl->p->type == CODELET_CPU)
		{
//		    fprintf(stderr, "in !\n");
		    tl->t->cpu  = starpu_find_fun_decl((pl->p->p.l)->name);
		    starpu_parse_cpu_function(tl->t->cpu->node, tl->t);
		    // parser la fonction cpu
		}
		else if(pl->p->type == CODELET_CUDA)
		{
		    tl->t->cuda = starpu_find_fun_decl((pl->p->p.l)->name);
		    starpu_parse_cuda_function(tl->t->cuda->node, tl->t);
		    // parser la fonction cuda
		}
	    }
	    tl->next = NULL;
	    if(starpu_task_list == NULL)
	    {
		starpu_task_list = tl;
	    }
	    else
	    {
		StarPU_task_list *tmp_l;
		for(tmp_l = starpu_task_list; tmp_l->next != NULL; tmp_l = tmp_l->next);
		tmp_l->next = tl;
	    }

        }else{
#if defined(DEBUG)
            printf("WARNING: probably there is something wrong with this starpu_insert_task().\n");
#endif
            return;
        }
	uint64_t var_rank = 0;
...
#else
		tmp->var_symname = starpu_symname(var_rank, tl->t);
#endif
		node_t *qual;
		symbolic_name_count++;
#ifdef QUARK_COMPILER
		qual = node->u.kids.kids[i+1];
#else
		qual = node->u.kids.kids[i-1];
#endif // QUARK_COMPILER
//		fprintf(stderr, "adding %s with %d\n", (tmp->u.kids.kids[0])->u.var_name, DA_quark_INOUT(qual));
	    ...
	    if( (i+1<node->u.kids.kid_count) && (i>1) 
		&& !strcmp(tree_to_str(node->u.kids.kids[i-1]), "SCRATCH"))
	    {
...
static void quark_record_uses_defs_and_pools(node_t *node){
    ...
    if( FCALL == node->type ){
	    ...
	    starpu_record_uses_defs_and_pools(node);
	    return;
        }
...
static matrix_variable_t *quark_find_all_matrices(node_t *node){
    ...
    int i;
    fprintf(stderr, "------>Entering quark_find_all_matrices\n");
    if( FCALL == node->type ){
        if( strcmp("QUARK_Insert_Task", DA_kid(node,0)->u.var_name) && strcmp("starpu_insert_task", DA_kid(node,0)->u.var_name)){
            return NULL;
        }
...
/*#else
    ...
    #endif*/
//    dump_all_unds();
    fprintf(stderr, "Interrogating omega...");
...
static void add_phony_INOUT_task_loops(matrix_variable_t *list, node_t *node, int task_type){
    ...
    for(curr = list; NULL != curr; curr = curr->next){
        ...
        if( TASK_IN == task_type ){
            asprintf(&(tmp_str), "CORE_PARSEC_IN_%s_quark", curr_matrix);
        }else if( TASK_OUT == task_type ){
            asprintf(&(tmp_str), "CORE_PARSEC_OUT_%s_quark", curr_matrix);
        }else{
            assert(0);
        }
...
void add_entry_and_exit_task_loops(node_t *node){
    ...
    fprintf(stderr, "entering entry exit task loop\n");
...
#else
	for(i=STARPU_FIRST_VAR; i<node->u.kids.kid_count; i++)
	{
	    
	    if((node->u.kids.kids[i])->type == IDENTIFIER && 
	       isArrayOut(node, i)                        && 
	       !isArrayIn(node, i))
	    {
		node_t *flag = node->u.kids.kids[i];
		flag = DA_create_B_expr(B_OR, flag, DA_create_ID("INPUT"));
		node->u.kids.kids[i] = flag;
	    }
...
static int DA_quark_INOUT(node_t *node){
    ...
    switch(node->type){
        case IDENTIFIER:
	    if( !strcmp(node->u.var_name, "STARPU_R") ){
                return UND_READ;
            }
            if( !strcmp(node->u.var_name, "STARPU_W") ){
                return UND_WRITE;
            }
	    if( !strcmp(node->u.var_name, "STARPU_RW") ){
		return UND_RW;
            }
...
static int isArrayLocal(node_t *task_node, int index){
    ...
    i = index;
#endif
    if( i < task_node->u.kids.kid_count && i > 0){
	node_t *flag = task_node->u.kids.kids[i];
...
node_t *starpu_print_default_task_placement(node_t *task_node){
    int i;
    for(i=STARPU_FIRST_VAR; i<task_node->u.kids.kid_count; i++)
    {
	if((task_node->u.kids.kids[i])->type == IDENTIFIER &&
	   isArrayOut(task_node, i) && isArrayLocal(task_node, i) )
	{
	    node_t *data_element = task_node->u.kids.kids[i+1];
    ...
    for(i=STARPU_FIRST_VAR; i<task_node->u.kids.kid_count; i++)
    {
        if( (task_node->u.kids.kids[i])->type == IDENTIFIER && isArrayOut(task_node, i) )
	{
            node_t *data_element = task_node->u.kids.kids[i+1];
            printf("  : %s\n",tree_to_str(data_element));
            return data_element;
 ...
 */
node_t *quark_print_default_task_placement(node_t *task_node){
    int i;
...
static char *size_to_pool_name(char *size_str){
    ...
    if( !pool_count )
        parsec_linked_list_construct(&_parsec_pool_list);

    /* See if a pool of this size exists already, and if so return it. */
    parsec_list_item_t *list_item = (parsec_list_item_t *)_parsec_pool_list.ghost_element.list_next;
    while(list_item != &(_parsec_pool_list.ghost_element) ){
        ...
        list_item = (parsec_list_item_t *)list_item->list_next;
    }
    ...
    parsec_linked_list_add_head( &_parsec_pool_list, (parsec_list_item_t *)new_item );
...
char *create_pool_declarations(){
    ...
    parsec_list_item_t *list_item = (parsec_list_item_t *)_parsec_pool_list.ghost_element.list_next;
    while(list_item && list_item != &(_parsec_pool_list.ghost_element) ){
        var_def_item_t *true_item = (var_def_item_t *)list_item;
        assert(list_item);
        ...
        result = append_to_string(result, true_item->var, " [type = \"parsec_memory_pool_t *\" size = \"%s\"]\n", 47+strlen(true_item->var));
...
char *quark_tree_to_body(node_t *node){
    ...
    parsec_linked_list_t var_def_list;
    parsec_linked_list_construct(&var_def_list);
...
#  endif
                ...
                tmp = NULL;
                parsec_list_item_t *item = (parsec_list_item_t *)var_def_list.ghost_element.list_next;
                ...
                if( NULL == tmp ){
                    ...
                    if( tmp != param ){
                        ...
                        tmp_item->var = param;
                        tmp_item->def = tmp;
                        PARSEC_LIST_ITEM_SINGLETON(tmp_item);
                        parsec_linked_list_add_head( &var_def_list, (parsec_list_item_t *)tmp_item );
                    }
                }
            ...
            //pool_push = append_to_string( pool_push, param, "  parsec_private_memory_push( %s", 35+strlen(param));
        ...
        if( ARRAY == node->u.kids.kids[i]->type ){
            ...
            // Mathieu said we should print the PaRSEC alias symbol, not the array base name.
    ...
    var_def_item_t *item;
    while( NULL != (item = (var_def_item_t *)parsec_linked_list_remove_head(&var_def_list)) ){
        free(item);
    }
