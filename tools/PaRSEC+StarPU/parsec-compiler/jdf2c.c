...
static void jdf_generate_header_file(const jdf_t* jdf)
{
    ...
    houtput("struct callback_args {\n"
	    "    parsec_execution_stream_t     *exec_unit;\n"
	    "    parsec_task_t  *task;\n"
	    "};\n");
    ...
    houtput("void generic_scheduling_func(parsec_execution_stream_t *context, parsec_list_item_t *elt);\n");
...
static void jdf_generate_structure(const jdf_t *jdf)
{
    ...
    coutput("/* Dependency Tracking Allocation Macro */\n"
            ...
            "  (DEPS)->dep_flags = PARSEC_DEPENDENCIES_FLAG_ALLOCATED | (FLAG);                                \\\n"
            "  PARSEC_STAT_INCREASE(mem_bitarray,  sizeof(parsec_dependencies_t) + STAT_MALLOC_OVERHEAD +   \\\n"
            "                   (_vmax - _vmin) * sizeof(parsec_dependencies_union_t));                    \\\n"
            "  (DEPS)->symbol = (vSYMBOL);                                                                \\\n"
            "  (DEPS)->min = _vmin;                                                                       \\\n"
            "  (DEPS)->max = _vmax;                                                                       \\\n"
                                     ...
                                     const char *condname, const jdf_def_list_t *context )
{
    ...
    string_arena_add_string(sa, 
                            "  .datatype_index = PARSEC_%s_%s_ARENA,"
                            "  .call_params = {\n",
                                   ...
                                   jdf_dataflow_t *flow, const char *prefix, uint32_t flow_index )
{
    ...
    assert((1<< flow_index) && (((1 << flow_index) & ~PARSEC_DEPENDENCIES_BITMASK) == 0)); 
    (void)flow_index;
#endif
    ...
    flow_flags = ((flow->flow_flags & JDF_FLOW_TYPE_CTL) ? "FLOW_ACCESS_NONE" :
    ...
    string_arena_add_string(sa, 
                            "static const parsec_flow_t %s%s = {\n"
                            "  .name = \"%s\",\n"
                            "  .sym_type = %s,\n"
                            "  .flow_flags = %s,\n"
                            "  .flow_index = %u,\n"
...
static void jdf_generate_startup_tasks(const jdf_t *jdf, const jdf_function_entry_t *f, const char *fname)
{
    ...
    assert( f->dep_flags & JDF_FUNCTION_FLAG_CAN_BE_STARTUP );
    (void)jdf;
    ...
    coutput("static int %s(parsec_context_t *context, const __parsec_%s_internal_taskpool_t *__tp, parsec_task_t** pready_list)\n"
            ...
            "%s\n"
            "%s\n"
            "  new_context = (parsec_task_t*)parsec_thread_mempool_allocate( context->execution_streams[0]->context_mempool );\n"
    ...
    coutput("#if defined(PARSEC_DEBUG_NOISIER)\n"
            "%s  {\n"
            "%s    char tmp[128];\n"
            "%s    PARSEC_DEBUG_VERBOSE(20, parsec_debug_output, \"Add startup task %%s\",\n"
            "%s           parsec_service_to_string(new_context, tmp, 128));\n"
            "%s  }\n"
    ...
    coutput("%s  new_context = (parsec_task_t*)parsec_thread_mempool_allocate( context->execution_streams[0]->context_mempool );\n"
    ...
    coutput("  parsec_thread_mempool_free( context->execution_streams[0]->context_mempool, new_context );\n");
...
static void jdf_generate_internal_init(const jdf_t *jdf, const jdf_function_entry_t *f, const char *fname)
{
    ...
    coutput("%s  if( !%s_pred(%s) ) continue;\n"
            ...
            indent(nesting));
    for(dl = f->definitions, pl = f->parameters; pl != NULL; dl = dl->next, pl = pl->next ) {
        coutput("%s  %s_max = parsec_imax(%s_max, %s);\n"
                "%s  %s_min = parsec_imin(%s_min, %s);\n",
    ...
    if( f->parameters->next == NULL ) {
        coutput("  if( 0 != nb_tasks ) {\n"
                "    ALLOCATE_DEP_TRACKING(dep, %s_min, %s_max, \"%s\", &symb_%s_%s_%s, NULL, PARSEC_DEPENDENCIES_FLAG_FINAL);\n"
                "  }\n",
        ...
        for(dl = f->definitions; dl != NULL; dl = dl->next) {
            ...
            if(dl->expr->op == JDF_RANGE) {
                ...
                coutput("%s  %s_end = %s;\n", 
                        indent(nesting), dl->name, dump_expr((void**)dl->expr->jdf_ba2, &info2));
                coutput("%s  for(%s = parsec_imax(%s_start, %s_min); %s <= parsec_imin(%s_end, %s_max); %s++) {\n",
...
static void jdf_generate_one_function( const jdf_t *jdf, const jdf_function_entry_t *f, int dep_index )
{
    ...
    string_arena_add_string(sa, 
                            ...
                            f->fname,
                            dep_index,
                            (f->dep_flags & JDF_FUNCTION_FLAG_HIGH_PRIORITY) ? "PARSEC_HIGH_PRIORITY_TASK" : "0x0",
...
static char *dump_pseudoparsec(void **elem, void *arg)
{
    string_arena_t *sa = (string_arena_t *)arg;
    char *name = *(char**)elem;
    string_arena_init(sa);
...
static void jdf_generate_predeclarations( const jdf_t *jdf )
{
    ...
    for(f = jdf->functions; f != NULL; f = f->next) {
	houtput("void hook_of_%s_%s_callback_function(parsec_execution_stream_t*, parsec_task_t*);\n", jdf_basename, f->fname);
	coutput("static int hook_of_%s_%s_task_create(parsec_execution_stream_t*, parsec_task_t*);\n",
		jdf_basename, f->fname); 
    ...
    for(f = jdf->functions; f != NULL; f = f->next) {
        for(fl = f->dataflow; fl != NULL; fl = fl->next) {
            ...
            coutput("static const parsec_flow_t flow_of_%s_%s_for_%s;\n", 
...
static void jdf_generate_destructor( const jdf_t *jdf )
{
    ...
    for( g = jdf->datatypes; NULL != g; g = g->next ) {
        coutput("  if( o->arenas[PARSEC_%s_%s_ARENA] != NULL ) {\n"
                "    parsec_arena_destruct(o->arenas[PARSEC_%s_%s_ARENA]);\n"
                "    free(o->arenas[PARSEC_%s_%s_ARENA]);\n"
                "  }\n",
                jdf_basename, g->name,
                jdf_basename, g->name,
                jdf_basename, g->name);
...
static void jdf_generate_constructor( const jdf_t* jdf )
{
            ...
            jdf_basename, jdf_basename);
    {
        ...
        for( g = jdf->datatypes; NULL != g; g = g->next ) {
            coutput("  _res->super.arenas[PARSEC_%s_%s_ARENA] = (parsec_arena_t*)calloc(1, sizeof(parsec_arena_t));\n",
                    jdf_basename, g->name);
        }
    }
                                                  ...
                                                  const char *spaces)
{
    ...
    if( call->var != NULL ) {
        ...
        coutput("%s  g%s = (parsec_arena_chunk_t*) %s(%s);\n",
...
jdf_generate_code_generic_scheduling_func(const jdf_t *jdf)
{
    ...
    coutput("void generic_scheduling_func(parsec_execution_stream_t *context, parsec_list_item_t *elt)\n"
	    "{\n");
    ...
    coutput("    parsec_task_t *task2exec;\n"
	    ...
	    "*/\n"
	    "        next = (parsec_list_item_t *)elt->list_next;\n"
	    "        if(next == elt) {\n"
            "            next = NULL;\n"
	    "        } else {\n"
...
{
    coutput("void %s_callback_function(parsec_execution_stream_t *exec_unit, parsec_task_t *task)\n"
	    "{\n"
	    ...
	    "        rc = task->task_class->complete_execution( exec_unit, task );\n\n"
	    ...
	    "    }\n"
	    "    DEBUG_MARK_EXE( eu_context->eu_id, task );\n"
	    "    PARSEC_STAT_DECREASE(mem_contexts, sizeof(parsec_task_t) + STAT_MALLOC_OVERHEAD);\n"
	    "    parsec_thread_mempool_free( exec_unit->context_mempool, task );\n"
...
jdf_generate_code_starpu_task_create(const jdf_t *jdf, const jdf_function_entry_t *f, const char *name)
{
    ...
    ai.holder = "this_task->locals";
    ai.expr = NULL;
    coutput("static int %s_task_create(parsec_execution_stream_t *context, parsec_task_t *this_task)\n"
            "{\n"
...
//	    "    __parsec_%s_internal_taskpool_t *obj_temp = malloc(sizeof(struct __parsec_%s_internal_handle));\n"
	    ...
	    /* "    callback_arg->exec_stream = context;\n" */
	    /* "    callback_arg->task       = this_task;\n" */
...
//	coutput("    starpu_data_set_sequential_consistency_flag(*(%s), 0);\n", fl->varname);
	...
	coutput("#if defined(PARSEC_SIM)\n"
                "  if( (NULL != e%s) && (e%s->sim_exec_date > __parsec_simulation_date) )\n"
                "    __parsec_simulation_date =  e%s->sim_exec_date;\n"
                "#endif\n",
                fl->varname,
                fl->varname,
                fl->varname);
...
static void jdf_generate_code_hook(const jdf_t *jdf, const jdf_function_entry_t *f, const char *name)
{
    ...
    /* coutput("static int %s(parsec_execution_stream_t *context, parsec_task_t *this_task)\n" */
    /*         "{\n" */
    ...
    coutput("static int complete_%s(parsec_execution_stream_t *context, parsec_task_t *this_task)\n"
            "{\n"
...
static void jdf_generate_code_release_deps(const jdf_t *jdf, const jdf_function_entry_t *f, const char *name)
{
    ...
    coutput("static int %s(parsec_execution_stream_t *eu, parsec_task_t *context, uint32_t action_mask, parsec_remote_deps_t *deps)\n"
            "{\n"
    ...
    coutput("  iterate_successors_of_%s_%s(eu, context, action_mask, parsec_release_dep_fct, &arg);\n"
            "\n",
            jdf_basename, f->fname);
    ...
    coutput("    if( NULL != arg.ready_list ) {\n"
            "      // Previous : __parsec_schedule(eu, arg.ready_list);\n"
	    "      //hook_of_%s_%s_task_create(eu, arg.ready_list);\n"
            "      generic_scheduling_func(eu, (parsec_list_item_t*)arg.ready_list);"
            "      arg.ready_list = NULL;\n"
            "    }\n"
            "  }\n",
            /* "#if defined(DISTRIBUTED)\n" */
            /* "  if( (action_mask & PARSEC_ACTION_SEND_REMOTE_DEPS) && arg.remote_deps_count ) {\n" */
            /* "    arg.nb_released += parsec_remote_dep_activate(eu, context, arg.remote_deps, arg.remote_deps_count);\n" */
            /* "  }\n" */
            /* "#endif\n", */
	    jdf_basename, f->fname);
                                         ...
                                         const char *var)
{
    ...
    string_arena_add_string(sa_open,
                            ...
                            "%s%s  PARSEC_DEBUG_VERBOSE(20, parsec_debug_output, \"thread %%d release deps of %s:%%s to %s:%%s (from node %%d to %%d)\", eu->eu_id,\n"
                            "%s%s         parsec_service_to_string(this_task, tmp, 128),\n"
                            "%s%s         parsec_service_to_string(&%s, tmp1, 128), rank_src, rank_dst);\n"
                            "%s%s}\n"
                            "#endif\n",
                            prefix, indent(nbopen),
                            prefix, indent(nbopen),
...
static void jdf_generate_code_iterate_successors(const jdf_t *jdf, const jdf_function_entry_t *f, const char *name)
{
    ...
    ai.expr = NULL;
    coutput("static void\n"
            "%s(parsec_execution_stream_t *eu, parsec_task_t *this_task,\n"
